...
export class PushNotificationService extends TypedEmitter<PushNotificationServiceEvents> {
    ...
    private async registerGcm(
        ...
        try {
            for (let retry_count = 1; retry_count <= retry; retry_count++) {
                rootPushLogger.debug(`Register GCM - Attempt ${retry_count} of ${retry}`, {
                    androidId: androidId,
                    fid: fid,
    ...
    private _normalizePushMessage(message: RawPushMessage): PushMessage {
        ...
        if (message.payload.payload) {
            ...
            if (normalizedMessage.type in ServerPushEvent) {
                ...
                if (normalizedMessage.type === DeviceType.FLOODLIGHT) normalizedMessage.device_sn = payload.station_sn;
                ...
                if (
                    ...
                    normalizedMessage.file_path =
                        platformPushData.file_path !== undefined ? platformPushData.file_path : "";
                    normalizedMessage.pic_url = platformPushData.pic_url !== undefined ? platformPushData.pic_url : "";
                    normalizedMessage.push_count =
                        platformPushData.push_count !== undefined ? platformPushData.push_count : 1;
                    ...
                    normalizedMessage.cipher =
                        batteryDoorbellPushData.cipher !== undefined ? batteryDoorbellPushData.cipher : 0;
                    normalizedMessage.event_session =
                    ...
                    normalizedMessage.push_count =
                        indoorPushData.push_count !== undefined ? indoorPushData.push_count : 1;
                    ...
                    normalizedMessage.storage_type =
                        indoorPushData.storage_type !== undefined ? indoorPushData.storage_type : 1;
                    ...
                    */
                    ...
                    normalizedMessage.short_user_id =
                        lockPushData.short_user_id !== undefined ? lockPushData.short_user_id : "";
                    ...
                    normalizedMessage.user_name =
                        garageDoorPushData.user_name !== undefined ? garageDoorPushData.user_name : "";
                    normalizedMessage.door_id =
                        garageDoorPushData.door_id !== undefined ? garageDoorPushData.door_id : -1;
                    normalizedMessage.name =
                        garageDoorPushData.door_name !== undefined ? garageDoorPushData.door_name : "";
                    normalizedMessage.pic_url =
                        garageDoorPushData.pic_url !== undefined ? garageDoorPushData.pic_url : "";
                    normalizedMessage.file_path =
                        garageDoorPushData.file_path !== undefined ? garageDoorPushData.file_path : "";
                    normalizedMessage.storage_type =
                        garageDoorPushData.storage_type !== undefined ? garageDoorPushData.storage_type : 1;
                    normalizedMessage.power =
                        garageDoorPushData.power !== undefined ? garageDoorPushData.power : undefined;
                } else {
                    ...
                    normalizedMessage.file_path =
                        cusPushData.c !== undefined && cusPushData.p !== undefined && cusPushData.p !== ""
                            ? getAbsoluteFilePath(normalizedMessage.type, cusPushData.c, cusPushData.p)
                    ...
                    try {
                        normalizedMessage.fetch_id =
                            cusPushData.i !== undefined ? Number.parseInt(cusPushData.i) : undefined;
                    } catch (err) {
                        const error = ensureError(err);
                        rootPushLogger.error(
                    ...
                    normalizedMessage.storage_type =
                        cusPushData.storage_type !== undefined ? cusPushData.storage_type : 1;
                    ...
                    if (Device.isStarlight4GLTE(normalizedMessage.type)) {
                        ...
                        try {
                            normalizedMessage.openType =
                                cusPushData.r !== undefined ? Number.parseInt(cusPushData.r) : 0;
                        } catch (err) {
                            const error = ensureError(err);
                            rootPushLogger.error(
                        ...
                        normalizedMessage.event_session =
                            cusPushData.session_id !== undefined ? cusPushData.session_id : "";
            ...
            if (doorbellPushData !== undefined) {
                ...
                normalizedMessage.push_count =
                    doorbellPushData.push_count !== undefined ? doorbellPushData.push_count : 1;
    ...
    private getCurrentPushRetryDelay(): number {
        ...
        if (this.retryDelay >= 60000 && this.retryDelay < 600000) this.retryDelay += 60000;
    ...
    private async _open(renew = false, forceNew = false): Promise<void> {
        ...
        if (this.credentials) {
            ...
            if (this.persistentIds) this.pushClient.setPersistentIds(this.persistentIds);
            ...
            this.connected = false;
            this.connecting = false;
            rootPushLogger.error("Push notifications are disabled, because the registration failed!", {
                credentials: this.credentials,
                renew: renew,
            });
        }
    ...
    public async open(): Promise<Credentials | undefined> {
        if (!this.connecting && !this.connected) {
            ...
            await this._open(false, true).catch((err) => {
                const error = ensureError(err);
                rootPushLogger.error(`Got exception trying to initialize push notifications`, {
                    error: getError(error),
                    credentials: this.credentials,
                });
            });
