...
export class PushNotificationService extends TypedEmitter<PushNotificationServiceEvents> {
    ...
    private async renewFidToken(
        ...
        try {
            ...
            if (response.statusCode == 200) {
                const result: FidTokenResponse =
                    response.body as FidTokenResponse;
                return {
                    ...result,
    ...
    private _normalizePushMessage(message: RawPushMessage): PushMessage {
        ...
        if (message.payload.payload) {
            ...
            if (normalizedMessage.type in ServerPushEvent) {
                ...
                switch (normalizedMessage.type) {
                    ...
                    case ServerPushEvent.ALARM_GUEST_NOTIFY:
                        ...
                        normalizedMessage.alarm_status =
                            alarmPushData.alarm_status;
                        normalizedMessage.alarm_action =
                            alarmPushData.alarm_action_channel;
                        try {
                ...
                if (
                    ...
                    normalizedMessage.cipher =
                        platformPushData.cipher !== undefined
                            ? platformPushData.cipher
                            : 0;
                    ...
                    normalizedMessage.tfcard_status =
                        platformPushData.tfcard_status;
                    normalizedMessage.user_type = platformPushData.user;
                    normalizedMessage.user_name = platformPushData.user_name;
                    normalizedMessage.station_guard_mode =
                        platformPushData.arming;
                    normalizedMessage.station_current_mode =
                        platformPushData.mode;
                    normalizedMessage.alarm_delay =
                        platformPushData.alarm_delay;
                    ...
                    const smartSafePushData =
                        payload.payload as SmartSafePushData;
                    normalizedMessage.event_type = smartSafePushData.event_type;
                    normalizedMessage.event_value =
                        smartSafePushData.event_value;
                    /*
                    event_value: {
                        type: 3,    3/4
                        action: 1,
                    ...
                    */
                    ...
                    normalizedMessage.push_count =
                        cusPushData.push_count !== undefined
                            ...
                            : 1;
                    normalizedMessage.notification_style =
                        cusPushData.notification_style;
            ...
            if (doorbellPushData !== undefined) {
                ...
                normalizedMessage.event_session =
                    doorbellPushData.event_session;
                ...
                normalizedMessage.doorbell_video_url =
                    doorbellPushData.video_url;
            }
        }
        ...
        if (!this.connecting && !this.connected) {
            ...
            if (!this.credentials) {
                ...
                this.retryTimeout = setTimeout(async () => {
                    rootPushLogger.info(
                        `Retry to register/login for push notification`
                    );
                    await this.open();
                }, delay);
            } else {
