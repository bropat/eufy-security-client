...
export class PushNotificationService extends TypedEmitter<PushNotificationServiceEvents> {
  ...
  private buildExpiresAt(expiresIn: string): number {
    if (expiresIn.endsWith("ms")) {
      ...
      return (
        new Date().getTime() +
        Number.parseInt(expiresIn.substring(0, expiresIn.length - 1)) * 1000
      );
    }
  ...
  private async registerFid(fid: string): Promise<FidInstallationResponse> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        const result: FidInstallationResponse =
          response.body as FidInstallationResponse;
        return {
          ...result,
          authToken: {
            ...result.authToken,
        ...
        rootPushLogger.error("Register FID - Status return code not 200", {
  ...
  private async renewFidToken(
    fid: string,
    refreshToken: string,
  ): Promise<FidTokenResponse> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        rootPushLogger.error("Renew FID Token - Status return code not 200", {
          status: response.statusCode,
          statusText: response.statusMessage,
          data: response.body,
        });
  ...
  private async renewPushCredentials(
    credentials: Credentials,
  ): Promise<Credentials> {
    return await this.renewFidToken(
      credentials.fidResponse.fid,
      credentials.fidResponse.refreshToken,
    )
      .then(async (response) => {
    ...
    try {
      for (let retry_count = 1; retry_count <= retry; retry_count++) {
        rootPushLogger.debug(
          `Register GCM - Attempt ${retry_count} of ${retry}`,
          { androidId: androidId, fid: fid, securityToken: securityToken },
        );
        ...
        if (response.statusCode == 200) {
          ...
          rootPushLogger.error("Register GCM - Status return code not 200", {
            status: response.statusCode,
            statusText: response.statusMessage,
            data: response.body,
          });
  ...
  private _normalizePushMessage(message: RawPushMessage): PushMessage {
    ...
    if (message.payload.payload) {
      ...
      try {
        ...
        const error = ensureError(err);
        rootPushLogger.error(`Normalize push message - type - Error`, {
          error: getError(error),
          message: message,
        });
      }
      ...
      if (normalizedMessage.type in ServerPushEvent) {
        ...
        switch (normalizedMessage.type) {
          ...
          case ServerPushEvent.ALARM_GUEST_NOTIFY:
            ...
            try {
              normalizedMessage.alarm_type = Number.parseInt(
                alarmPushData.alarm_id,
              );
            } catch (err) {
              const error = ensureError(err);
              rootPushLogger.error(
                `Normalize push message - alarm_type - Error`,
            ...
            try {
              normalizedMessage.event_time =
                alarmPushData.alert_time !== undefined
                  ? convertTimestampMs(alarmPushData.alert_time)
                  : Number.parseInt(alarmPushData.alert_time);
            } catch (err) {
              const error = ensureError(err);
              rootPushLogger.error(
                `Normalize push message - event_time - Error`,
                { error: getError(error), message: message },
              );
            }
            break;
        ...
        try {
          normalizedMessage.event_time =
            payload.event_time !== undefined
              ? convertTimestampMs(Number.parseInt(payload.event_time))
              : Number.parseInt(payload.event_time);
        } catch (err) {
          const error = ensureError(err);
          rootPushLogger.error(
            `Normalize push message - Type ${DeviceType[normalizedMessage.type]} CusPush - event_time - Error`,
            { error: getError(error), message: message },
          );
        }
        try {
          normalizedMessage.push_time =
            payload.push_time !== undefined
              ? convertTimestampMs(Number.parseInt(payload.push_time))
              : Number.parseInt(payload.push_time);
        } catch (err) {
          const error = ensureError(err);
          rootPushLogger.error(
            `Normalize push message - Type ${DeviceType[normalizedMessage.type]} CusPush - push_time - Error`,
            { error: getError(error), message: message },
          );
        }
        ...
        else normalizedMessage.device_sn = payload.device_sn;
        if (
          isEmpty(normalizedMessage.device_sn) &&
          !isEmpty(normalizedMessage.station_sn)
        ) {
        ...
        if (
          ...
          normalizedMessage.channel =
            platformPushData.channel !== undefined
              ...
              : 0;
          normalizedMessage.cipher =
            platformPushData.cipher !== undefined ? platformPushData.cipher : 0;
          ...
          normalizedMessage.push_count =
            platformPushData.push_count !== undefined
              ...
              : 1;
          normalizedMessage.notification_style =
            platformPushData.notification_style;
          normalizedMessage.storage_type =
            platformPushData.storage_type !== undefined
              ? platformPushData.storage_type
              : 1;
          ...
          normalizedMessage.sound_alarm =
            platformPushData.alarm !== undefined
              ? platformPushData.alarm === 1
                ? true
                : false
              : undefined;
        } else if (
          Device.isBatteryDoorbell(normalizedMessage.type) ||
          Device.isWiredDoorbellDual(normalizedMessage.type)
        ) {
          const batteryDoorbellPushData =
          ...
          normalizedMessage.channel =
            batteryDoorbellPushData.channel !== undefined
              ...
              : 0;
          normalizedMessage.cipher =
            batteryDoorbellPushData.cipher !== undefined
              ? batteryDoorbellPushData.cipher
              : 0;
          normalizedMessage.event_session =
          ...
          normalizedMessage.push_count =
            batteryDoorbellPushData.push_count !== undefined
              ...
              : 1;
          normalizedMessage.notification_style =
            batteryDoorbellPushData.notification_style;
        } else if (
          Device.isIndoorCamera(normalizedMessage.type) ||
          ...
          Device.isFloodLightT8420X(
            normalizedMessage.type,
            normalizedMessage.device_sn,
          ) ||
          (Device.isFloodLight(normalizedMessage.type) &&
            normalizedMessage.type !== DeviceType.FLOODLIGHT)
        ) {
          ...
          normalizedMessage.pic_url =
            indoorPushData.pic_url !== undefined ? indoorPushData.pic_url : "";
          normalizedMessage.push_count =
            indoorPushData.push_count !== undefined
              ? indoorPushData.push_count
              : 1;
          normalizedMessage.notification_style =
            indoorPushData.notification_style;
          ...
          normalizedMessage.storage_type =
            indoorPushData.storage_type !== undefined
              ? indoorPushData.storage_type
              : 1;
          ...
          Device.isLock(normalizedMessage.type) &&
          !Device.isLockWifiVideo(normalizedMessage.type)
        ) {
          ...
          normalizedMessage.user_id =
            lockPushData.user_id !== undefined ? lockPushData.user_id : "";
          normalizedMessage.name =
            lockPushData.device_name !== undefined
              ? lockPushData.device_name
              : "";
          normalizedMessage.person_name =
            lockPushData.nick_name !== undefined ? lockPushData.nick_name : "";
          ...
          normalizedMessage.storage_type =
            garageDoorPushData.storage_type !== undefined
              ...
              : 1;
          normalizedMessage.power =
            garageDoorPushData.power !== undefined
              ? garageDoorPushData.power
              : undefined;
        } else {
          ...
          normalizedMessage.file_path =
            ...
            cusPushData.p !== ""
              ...
              : cusPushData.file_path
                ...
                : "";
          normalizedMessage.pic_url =
            cusPushData.pic_url !== undefined ? cusPushData.pic_url : "";
          normalizedMessage.push_count =
            cusPushData.push_count !== undefined ? cusPushData.push_count : 1;
          ...
          normalizedMessage.device_online =
            cusPushData.m !== undefined
              ? cusPushData.m === 1
                ? true
                : false
              : undefined;
          try {
            normalizedMessage.fetch_id =
              cusPushData.i !== undefined
                ? Number.parseInt(cusPushData.i)
                : undefined;
          } catch (err) {
            const error = ensureError(err);
            rootPushLogger.error(
              `Normalize push message - Type ${DeviceType[normalizedMessage.type]} CusPushData - fetch_id - Error`,
              { error: getError(error), message: message },
            );
          }
          normalizedMessage.sense_id = cusPushData.j;
          normalizedMessage.battery_powered =
            cusPushData.batt_powered !== undefined
              ? cusPushData.batt_powered === 1
                ? true
                : false
              : undefined;
          try {
            normalizedMessage.battery_low =
              cusPushData.bat_low !== undefined
                ? Number.parseInt(cusPushData.bat_low)
                : undefined;
          } catch (err) {
            const error = ensureError(err);
            rootPushLogger.error(
              `Normalize push message - Type ${DeviceType[normalizedMessage.type]} CusPushData - battery_low - Error`,
              { error: getError(error), message: message },
            );
          }
          normalizedMessage.storage_type =
            cusPushData.storage_type !== undefined
              ? cusPushData.storage_type
              : 1;
          ...
          if (Device.isStarlight4GLTE(normalizedMessage.type)) {
            if (
              cusPushData.channel &&
              cusPushData.channel !== null &&
              cusPushData.channel !== undefined
            ) {
              normalizedMessage.channel = cusPushData.channel;
            }
            if (
              cusPushData.cipher &&
              cusPushData.cipher !== null &&
              cusPushData.cipher !== undefined
            ) {
              normalizedMessage.cipher = cusPushData.cipher;
            }
            if (
              cusPushData.event_type &&
              cusPushData.event_type !== null &&
              cusPushData.event_type !== undefined
            ) {
              normalizedMessage.event_type = cusPushData.event_type;
            }
            if (
              cusPushData.file_path &&
              cusPushData.file_path !== null &&
            ...
            try {
              normalizedMessage.open =
                cusPushData.e !== undefined
                  ? Number.parseInt(cusPushData.e)
                  : 0;
            } catch (err) {
              const error = ensureError(err);
              rootPushLogger.error(
                `Normalize push message - Type ${DeviceType[normalizedMessage.type]} CusPushData - open - Error`,
                { error: getError(error), message: message },
              );
            }
            try {
              normalizedMessage.openType =
                cusPushData.r !== undefined
                  ? Number.parseInt(cusPushData.r)
                  : 0;
            } catch (err) {
              const error = ensureError(err);
              rootPushLogger.error(
                `Normalize push message - Type ${DeviceType[normalizedMessage.type]} CusPushData - openType - Error`,
                { error: getError(error), message: message },
              );
            }
            ...
            normalizedMessage.channel =
              cusPushData.channel !== undefined ? cusPushData.channel : 0;
            normalizedMessage.cipher =
              cusPushData.cipher !== undefined ? cusPushData.cipher : 0;
            normalizedMessage.event_session =
              cusPushData.session_id !== undefined
                ? cusPushData.session_id
                : "";
            normalizedMessage.file_path =
              cusPushData.file_path !== undefined ? cusPushData.file_path : "";
          }
        }
      }
    } else if (message.payload.doorbell !== undefined) {
      const doorbellPushData = parseJSON(
        message.payload.doorbell,
        rootPushLogger,
      ) as DoorbellPushData;
      if (doorbellPushData !== undefined) {
        ...
        normalizedMessage.event_time =
          doorbellPushData.create_time !== undefined
            ? convertTimestampMs(doorbellPushData.create_time)
        ...
        normalizedMessage.push_time =
          doorbellPushData.event_time !== undefined
            ? convertTimestampMs(doorbellPushData.event_time)
        ...
        normalizedMessage.push_count =
          doorbellPushData.push_count !== undefined
            ? doorbellPushData.push_count
            : 1;
  ...
  private getCurrentPushRetryDelay(): number {
    ...
    if (this.retryDelay < 60000) this.retryDelay += 10000;
  ...
  private async _open(renew = false, forceNew = false): Promise<void> {
    if (
      ...
      (this.credentials &&
        ...
        new Date().getTime() >=
          this.credentials.fidResponse.authToken.expiresAt)
    ) {
      rootPushLogger.debug(`Create new push credentials...`, {
      ...
      rootPushLogger.debug(`Renew push credentials...`, {
        credentials: this.credentials,
        renew: renew,
      });
      this.credentials = await this.renewPushCredentials(
        this.credentials,
      ).catch((err) => {
        const error = ensureError(err);
      ...
      rootPushLogger.debug(`Login with previous push credentials...`, {
        credentials: this.credentials,
      });
      this.credentials = await this.loginPushCredentials(
        this.credentials,
      ).catch((err) => {
        const error = ensureError(err);
  ...
  public async open(): Promise<Credentials | undefined> {
    if (!this.connecting && !this.connected) {
      ...
      await this._open(false, true).catch((err) => {
        const error = ensureError(err);
        rootPushLogger.error(
          `Got exception trying to initialize push notifications`,
          { error: getError(error), credentials: this.credentials },
        );
      });
      ...
      if (!this.credentials) {
        ...
        rootPushLogger.info(
          `Retry to register/login for push notification in ${delay / 1000} seconds...`,
        );
