...
export class EufySecurity extends TypedEmitter<EufySecurityEvents> {
  ...
  private cameraStationLivestreamTimeout: Map<string, NodeJS.Timeout> = new Map<
    string,
    NodeJS.Timeout
  >();
  ...
  protected async _initializeInternals(): Promise<void> {
    ...
    if (this.config.p2pConnectionSetup === undefined) {
      ...
      !Object.values(P2PConnectionType).includes(this.config.p2pConnectionSetup)
    ) {
      this.config.p2pConnectionSetup = P2PConnectionType.QUICKEST;
    }
    ...
    try {
      if (
        !this.config.persistentData &&
        fse.statSync(this.persistentFile).isFile()
      ) {
        ...
        promises.push(
          station.then((station: Station) => {
            try {
              ...
              station.on(
                "livestream error",
                (station: Station, channel: number, error: Error) =>
              ...
              station.on(
                ...
                (station: Station, type: number, value: string) =>
              ...
              station.on(
                "wifi rssi",
                (station: Station, channel: number, rssi: number) =>
                  this.onStationWifiRssi(station, channel, rssi),
              );
              station.on(
                "floodlight manual switch",
                (station: Station, channel: number, enabled: boolean) =>
              ...
              station.on(
                "image download",
                (station: Station, file: string, image: Buffer) =>
              ...
              station.on(
                "sensor status",
                (station: Station, channel: number, status: number) =>
        ...
        this.getStation(device.getStationSerial())
          ...
          .catch((err) => {
            ...
            rootMainLogger.error(
              ...
              { error: getError(error), deviceSN: device.getSerial() },
            );
          });
      });
    ...
    if (Object.keys(houseInvites).length > 0) {
      for (const invite of Object.values(
        ...
        const result = await this.api
          ...
          .catch((err) => {
            const error = ensureError(err);
            rootMainLogger.error("Error in confirmation of house invitations", {
              error: getError(error),
            });
            return error;
          });
  ...
  private onPushMessage(message: PushMessage): void {
    ...
    try {
      ...
      this.getStations()
        ...
        .catch((err) => {
          const error = ensureError(err);
          rootMainLogger.error("Process push notification for stations", {
            error: getError(error),
            message: message,
          });
        });
      this.getDevices()
        ...
        .catch((err) => {
          const error = ensureError(err);
          rootMainLogger.error("Process push notification for devices", {
            error: getError(error),
            message: message,
          });
        });
        ...
        .then((device: Device) => {
          if (
            ...
            if (device.hasProperty(result.customData.property.name)) {
              ...
              if (
                typeof result.customData.property.value !== "object" ||
            ...
            const snoozeTime =
              ...
              result.customData.command.value.snooze_time !== undefined
            ...
            this.api
              ...
              .then(() => {
                ...
                if (snoozeStartTime !== undefined && snoozeStartTime !== 0) {
                  timeoutMS =
                    (snoozeStartTime + snoozeTime - currentTime) * 1000;
                } else {
    ...
    if (
      ...
      switch (result.customData.command.name) {
        case CommandName.DeviceAddUser:
          this.getStationDevice(station.getSerial(), result.channel).then(
            (device: Device) => {
  ...
  private onDeviceReady(device: Device): void {
    try {
      if (
        ...
        (device.getPropertyValue(PropertyName.DeviceRTSPStream) as boolean) ===
          true
      ) {
        this.getStation(device.getStationSerial())
          .then((station: Station) => {
    ...
    this.getStationDevice(station.getSerial(), channel)
      .then((device: Device) => {
        ...
        if (device.hasProperty(PropertyName.DeviceBatteryTemp)) {
          const metadataBatteryTemperature = device.getPropertyMetadata(
