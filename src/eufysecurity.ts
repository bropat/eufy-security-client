...
export class EufySecurity extends TypedEmitter<EufySecurityEvents> {
    ...
    private cameraStationLivestreamTimeout: Map<string, NodeJS.Timeout> =
    ...
    protected async _initializeInternals(): Promise<void> {
            ...
            if (
                ...
                for (const category of this.config.logging.categories) {
                    if (
                        ...
                        setLoggingLevel(
                            category.category.toLocaleLowerCase() as LoggingCategories,
                            category.level
                        );
                    }
                }
        ...
        if (this.config.p2pConnectionSetup === undefined) {
            ...
            !Object.values(P2PConnectionType).includes(
        ...
        try {
            if (this.persistentData.version !== libVersion) {
                ...
                if (previousVersion < currentVersion) {
                    this.persistentData = handleUpdate(
                        this.persistentData,
                        previousVersion
                    );
        ...
        if (
            ...
            if (this.persistentData.fallbackTrustedDeviceName !== undefined) {
                this.config.trustedDeviceName =
                    this.persistentData.fallbackTrustedDeviceName;
            } else {
        ...
        if (
            ...
            if (
                md5(`${this.config.username}:${this.config.password}`) !=
                this.persistentData.login_hash
            ) {
                rootMainLogger.info(
        ...
        if (
            ...
            this.api.setTokenExpiration(
                new Date(this.persistentData.cloud_token_expiration)
            );
        }
        if (
            ...
            this.getDevice(message.data.data.deviceSn)
                .then((device: Device) => {
                    (device as Lock).processMQTTNotification(
                        message.data.data,
                        this.config.eventDurationSeconds
                    );
                })
                .catch((err) => {
                    const error = ensureError(err);
    ...
    private async updateStation(hub: StationListResponse): Promise<void> {
        ...
        if (Object.keys(this.stations).includes(hub.station_sn)) {
            ...
            if (
                ...
                this.stations[hub.station_sn].setConnectionType(
                    this.config.p2pConnectionSetup
                );
                rootMainLogger.debug(
                    `Updating station cloud data - initiate station connection to get local data over p2p`,
            ...
            rootMainLogger.debug(
                `Station with this serial ${hub.station_sn} doesn't exists and couldn't be updated!`
            );
        }
    }
    ...
    private addDevice(device: Device): void {
        ...
        if (serial && !Object.keys(this.devices).includes(serial)) {
            ...
            rootMainLogger.debug(
                `Device with this serial ${device.getSerial()} exists already and couldn't be added again!`
            );
        }
    }
    ...
    private async updateDevice(device: DeviceListResponse): Promise<void> {
        ...
        if (Object.keys(this.devices).includes(device.device_sn))
            ...
            rootMainLogger.debug(
                `Device with this serial ${device.device_sn} doesn't exists and couldn't be updated!`
            );
    }
    ...
    public async getDevicesFromStation(
        stationSN: string
    ...
    public async getDevice(deviceSN: string): Promise<Device> {
        ...
        throw new DeviceNotFoundError("Device doesn't exists", {
            context: { device: deviceSN },
        });
    }

    public async getStationDevice(
        stationSN: string,
        channel: number
    ): Promise<Device> {
        if (this.devicesLoaded) await this.devicesLoaded;
        for (const device of Object.values(this.devices)) {
            if (
                (device.getStationSerial() === stationSN &&
                    device.getChannel() === channel) ||
                (device.getStationSerial() === stationSN &&
                    device.getSerial() === stationSN)
            ) {
                return device;
            }
        }
    ...
    public async getStation(stationSN: string): Promise<Station> {
        ...
        throw new StationNotFoundError("Station doesn't exists", {
            context: { station: stationSN },
        });
    }
    ...
    public async connectToStation(
        ...
        if (station.isP2PConnectableDevice()) {
            ...
            rootMainLogger.debug(
                `Explicit request for p2p connection to the station`,
    ...
    public async isStationEnergySavingDevice(
        stationSN: string
    ): Promise<boolean> {
        const station = await this.getStation(stationSN);
    ...
    private handleHubs(hubs: Hubs): void {
        ...
        for (const hub of Object.values(hubs)) {
            if (stationsSNs.includes(hub.station_sn)) {
                ...
                const station = Station.getInstance(
                    this.api,
                    hub,
                    ipAddress,
                    0,
                    this.config.enableEmbeddedPKCS1Support
                );
                promises.push(
                        ...
                        try {
                            ...
                            station.on(
                                ...
                                (
                                    ...
                                    this.onStationRawPropertyChanged(
                                        station,
                                        type,
                                        value
                                    )
                            ...
                            station.on(
                                ...
                                (
                                    ...
                                    this.onStationDeviceShakeAlarm(
                                        deviceSN,
                                        event
                                    )
                            );
                            ...
                            station.on(
                                ...
                                (
                                    ...
                                    this.onStationDatabaseQueryLatest(
                                        station,
                                        returnCode,
                                        data
                                    )
                            ...
                            station.on(
                                ...
                                (
                                    ...
                                    this.onStationDatabaseQueryLocal(
                                        station,
                                        returnCode,
                                        data
                                    )
                            ...
                            station.on(
                                ...
                                (
                                    ...
                                    this.onStationDatabaseCountByDate(
                                        station,
                                        returnCode,
                                        data
                                    )
                            ...
                            station.on(
                                ...
                                (
                                    ...
                                    this.onStationGarageDoorStatus(
                                        station,
                                        channel,
                                        doorId,
                                        status
    ...
    private refreshP2PData(station: Station): void {
        if (
            ...
            (Device.isCamera(station.getDeviceType()) &&
                !Device.isWiredDoorbell(station.getDeviceType())) ||
        ...
        if (
            ...
            (station.hasProperty(PropertyName.StationSdStatus) &&
                station.getPropertyValue(PropertyName.StationSdStatus) !==
                    TFCardStatus.REMOVE)
        ) {
        ...
        for (const device_sn of this.cameraStationLivestreamTimeout.keys()) {
            this.getDevice(device_sn)
                .then((device: Device) => {
                    if (
                        device !== null &&
                        device.getStationSerial() === station.getSerial()
                    ) {
                        clearTimeout(
                            this.cameraStationLivestreamTimeout.get(device_sn)!
                        );
                ...
                .catch((err) => {
                    const error = ensureError(err);
                    rootMainLogger.error(`Station close Error`, {
                        error: getError(error),
                        stationSN: station.getSerial(),
                    });
                });
        ...
        for (const device of Object.values(devices)) {
            if (deviceSNs.includes(device.device_sn)) {
                ...
                if (Device.isIndoorCamera(device.device_type)) {
                    new_device = IndoorCamera.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isSoloCameras(device.device_type)) {
                    new_device = SoloCamera.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isLockWifiVideo(device.device_type)) {
                    new_device = DoorbellLock.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isBatteryDoorbell(device.device_type)) {
                    new_device = BatteryDoorbellCamera.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (
                    Device.isWiredDoorbell(device.device_type) ||
                    Device.isWiredDoorbellDual(device.device_type)
                ) {
                    new_device = WiredDoorbellCamera.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isFloodLight(device.device_type)) {
                    new_device = FloodlightCamera.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isWallLightCam(device.device_type)) {
                    new_device = WallLightCam.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isGarageCamera(device.device_type)) {
                    new_device = GarageCamera.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isSmartDrop(device.device_type)) {
                    new_device = SmartDrop.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isCamera(device.device_type)) {
                    new_device = Camera.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isLock(device.device_type)) {
                    new_device = Lock.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isMotionSensor(device.device_type)) {
                    new_device = MotionSensor.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isEntrySensor(device.device_type)) {
                    new_device = EntrySensor.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isKeyPad(device.device_type)) {
                    new_device = Keypad.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isSmartSafe(device.device_type)) {
                    new_device = SmartSafe.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isSmartTrack(device.device_type)) {
                    new_device = Tracker.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else if (Device.isLockKeypad(device.device_type)) {
                    new_device = LockKeypad.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
                } else {
                    new_device = UnknownDevice.getInstance(
                        this.api,
                        device,
                        deviceConfig
                    );
            ...
            devices.forEach((device) => {
                this.getStation(device.getStationSerial())
                    .then((station: Station) => {
                        if (
                            !station.isConnected() &&
                            station.isP2PConnectableDevice()
                        ) {
                            station.setConnectionType(
                                this.config.p2pConnectionSetup
                            );
                            rootMainLogger.debug(
                                `Initiate first station connection to get data over p2p`,
                                { stationSN: station.getSerial() }
                            );
                            station.connect();
                        }
    ...
    public close(): void {
        ...
        Object.keys(this.refreshEufySecurityP2PTimeout).forEach(
            (station_sn) => {
        ...
        await this.api
            ...
            .then(async () => {
                if (options?.verifyCode) {
                    ...
                    if (!trusted)
                        return await this.api.addTrustDevice(
                            options?.verifyCode
                        );
                }
            })
    ...
    private async onAPIConnect(): Promise<void> {
        ...
        this.registerPushNotifications(
            ...
            this.persistentData.push_persistentIds
        );
        ...
        if (!device.hasCommand(CommandName.DeviceStartLivestream))
            throw new NotSupportedError(
                "This functionality is not implemented or supported by this device",
                {
                    context: {
                        device: deviceSN,
                        commandName: CommandName.DeviceStartLivestream,
                    },
                }
            );
        ...
        if (!device.hasCommand(CommandName.DeviceStopLivestream))
            throw new NotSupportedError(
                "This functionality is not implemented or supported by this device",
                {
                    context: {
                        device: deviceSN,
                        commandName: CommandName.DeviceStopLivestream,
                    },
                }
            );
    ...
    private writePersistentData(): void {
        this.persistentData.login_hash = md5(
    ...
    private async processInvitations(): Promise<void> {
        ...
        const houseInvites = await this.api
            .getHouseInviteList()
            .catch((err) => {
                const error = ensureError(err);
        ...
        if (Object.keys(houseInvites).length > 0) {
            for (const invite of Object.values(
                houseInvites
            ) as HouseInviteListResponse[]) {
                const result = await this.api
                    .confirmHouseInvite(invite.house_id, invite.id)
                    .catch((err) => {
                        const error = ensureError(err);
                        rootMainLogger.error(
                ...
                if (result) {
                    rootMainLogger.info(
                        `Accepted received house invitation from ${invite.action_user_email}`,
                        { invite: invite }
                    );
                    refreshCloud = true;
                }
    ...
    private onPushMessage(message: PushMessage): void {
        ...
        try {
            ...
            try {
                if (
                    (message.type === ServerPushEvent.INVITE_DEVICE ||
                        message.type === ServerPushEvent.HOUSE_INVITE) &&
                ...
                const error = ensureError(err);
                rootMainLogger.error(
                    `Error processing server push notification for device invitation`,
            ...
            try {
                ...
                const error = ensureError(err);
                rootMainLogger.error(
                    `Error processing server push notification for device/station/house removal`,
                    { error: getError(error), message: message }
                );
            }
            this.getStations()
                .then((stations: Station[]) => {
                    stations.forEach((station) => {
                        try {
                            ...
                            const error = ensureError(err);
                            rootMainLogger.error(
                                `Error processing push notification for station`,
                                {
                                    error: getError(error),
                                    stationSN: station.getSerial(),
                                    message: message,
                ...
                .then((devices: Device[]) => {
                    devices.forEach((device) => {
                        this.getStation(device.getStationSerial())
                            .then((station) => {
                                try {
                                    device.processPushNotification(
                                        station,
                                        message,
                                        this.config.eventDurationSeconds
                                    );
                                    ...
                                    const error = ensureError(err);
                                    rootMainLogger.error(
                                        `Error processing push notification for device`,
                                        {
                                            error: getError(error),
                                            deviceSN: device.getSerial(),
                                            message: message,
    ...
    public async startStationDownload(
        deviceSN: string,
        path: string,
        cipherID: number
    ): Promise<void> {
        ...
        if (!device.hasCommand(CommandName.DeviceStartDownload))
            throw new NotSupportedError(
                "This functionality is not implemented or supported by this device",
                {
                    context: {
                        device: deviceSN,
                        commandName: CommandName.DeviceStartDownload,
                        path: path,
                        cipherID: cipherID,
                    },
                }
        ...
        if (!station.isDownloading(device)) {
            ...
            rootMainLogger.warn(
                `The station is already downloading a video for the device ${deviceSN}!`
            );
        }
    }
    ...
    public async cancelStationDownload(deviceSN: string): Promise<void> {
        ...
        if (!device.hasCommand(CommandName.DeviceCancelDownload))
            throw new NotSupportedError(
                "This functionality is not implemented or supported by this device",
                {
                    context: {
                        device: deviceSN,
                        commandName: CommandName.DeviceCancelDownload,
                    },
                }
            );
        ...
        if (station.isConnected() && station.isDownloading(device)) {
            ...
            rootMainLogger.warn(
                `The station isn't downloading a video for the device ${deviceSN}!`
            );
        }
    }
    ...
    public async setDeviceProperty(
        ...
        switch (name) {
            ...
            case PropertyName.DeviceLightSettingsEnable:
                station.setFloodlightLightSettingsEnable(
                    device,
                    value as boolean
                );
                break;
            ...
            case PropertyName.DeviceLightSettingsBrightnessMotion:
                station.setFloodlightLightSettingsBrightnessMotion(
                    device,
                    value as number
                );
                break;
            case PropertyName.DeviceLightSettingsBrightnessSchedule:
                station.setFloodlightLightSettingsBrightnessSchedule(
                    device,
                    value as number
                );
                break;
            case PropertyName.DeviceLightSettingsMotionTriggered:
                station.setFloodlightLightSettingsMotionTriggered(
                    device,
                    value as boolean
                );
                break;
            case PropertyName.DeviceLightSettingsMotionTriggeredDistance:
                station.setFloodlightLightSettingsMotionTriggeredDistance(
                    device,
                    value as number
                );
                break;
            case PropertyName.DeviceLightSettingsMotionTriggeredTimer:
                station.setFloodlightLightSettingsMotionTriggeredTimer(
                    device,
                    value as number
                );
                break;
            ...
            case PropertyName.DeviceRecordingEndClipMotionStops:
                station.setRecordingEndClipMotionStops(
                    device,
                    value as boolean
                );
                break;
            ...
            case PropertyName.DeviceNotificationAllOtherMotion:
                if (device.isIndoorPanAndTiltCameraS350()) {
                    ...
                    station.setNotificationAllOtherMotion(
                        device,
                        value as boolean
                    );
                }
            ...
            case PropertyName.DeviceMotionDetectionRangeStandardSensitivity:
                station.setMotionDetectionRangeStandardSensitivity(
                    device,
                    value as number
                );
                break;
            case PropertyName.DeviceMotionDetectionRangeAdvancedLeftSensitivity:
                station.setMotionDetectionRangeAdvancedLeftSensitivity(
                    device,
                    value as number
                );
                break;
            case PropertyName.DeviceMotionDetectionRangeAdvancedMiddleSensitivity:
                station.setMotionDetectionRangeAdvancedMiddleSensitivity(
                    device,
                    value as number
                );
                break;
            case PropertyName.DeviceMotionDetectionRangeAdvancedRightSensitivity:
                station.setMotionDetectionRangeAdvancedRightSensitivity(
                    device,
                    value as number
                );
                break;
            ...
            case PropertyName.DeviceRingAutoResponseVoiceResponse:
                station.setRingAutoResponseVoiceResponse(
                    device,
                    value as boolean
                );
                break;
            case PropertyName.DeviceRingAutoResponseVoiceResponseVoice:
                station.setRingAutoResponseVoiceResponseVoice(
                    device,
                    value as number
                );
                break;
            ...
            case PropertyName.DeviceDeliveryGuardUncollectedPackageAlertTimeToCheck:
                station.setDeliveryGuardUncollectedPackageAlertTimeToCheck(
                    device,
                    value as string
                );
                break;
            ...
            case PropertyName.DeviceMotionDetectionTypeHumanRecognition:
                station.setMotionDetectionTypeHB3(
                    device,
            ...
            case PropertyName.DeviceMotionDetectionTypeHuman:
                if (device.isWallLightCam()) {
                    station.setMotionDetectionTypeHuman(
                        device,
                        value as boolean
                    );
            ...
            case PropertyName.DeviceMotionDetectionTypeAllOtherMotions:
                if (device.isWallLightCam()) {
                    station.setMotionDetectionTypeAllOtherMotions(
                        device,
                        value as boolean
                    );
                } else if (device.isOutdoorPanAndTiltCamera()) {
            ...
            case PropertyName.DeviceLeftBehindAlarm: {
                ...
                const result = await tracker.setLeftBehindAlarm(
                    value as boolean
                );
                if (result) {
            ...
            case PropertyName.DeviceLeavingReactionNotification:
                station.setLeavingReactionNotification(
                    device,
                    value as boolean
                );
                break;
    ...
    public async setStationProperty(
        stationSN: string,
        name: string,
        value: unknown
    ): Promise<void> {
        ...
        switch (name) {
            ...
            case PropertyName.StationNotificationSwitchModeApp:
                station.setStationNotificationSwitchMode(
            ...
            case PropertyName.StationCrossTrackingGroupList:
                station.setCrossTrackingGroupList(
                    value as Array<CrossTrackingGroupEntry>
                );
                break;
            default:
                if (!Object.values(PropertyName).includes(name as PropertyName))
    ...
    private onStationCommandResult(
        ...
        if (result.return_code === 0) {
            if (
                ...
                result.customData.onSuccess !== undefined
            ) {
                try {
                    ...
                    const error = ensureError(err);
                    rootMainLogger.error(
                        `Station command result - onSuccess callback error`,
                        {
                            error: getError(error),
                            stationSN: station.getSerial(),
                            result: result,
            ...
            this.getStationDevice(station.getSerial(), result.channel)
                .then((device: Device) => {
                    if (
                        ...
                        if (
                            device.hasProperty(result.customData.property.name)
                        ) {
                            ...
                            if (
                                ...
                                device.updateProperty(
                                    result.customData.property.name,
                                    result.customData.property.value
                                );
                            }
                        } else if (
                            station.hasProperty(result.customData.property.name)
                        ) {
                            const metadata = station.getPropertyMetadata(
                                result.customData.property.name
                            );
                            if (
                                typeof result.customData.property.value !==
                        ...
                        result.customData.command.name ===
                            CommandName.DeviceSnooze
                    ) {
                        const snoozeTime =
                            result.customData.command.value !== undefined &&
                            result.customData.command.value.snooze_time !==
                                undefined
                                ? (result.customData.command.value
                                      .snooze_time as number)
                                : 0;
                        if (snoozeTime > 0) {
                            device.updateProperty(
                                PropertyName.DeviceSnooze,
                                true
                            );
                            device.updateProperty(
                        ...
                        this.api
                            .refreshAllData()
                            .then(() => {
                                const snoozeStartTime = device.getPropertyValue(
                                ...
                                this.deviceSnoozeTimeout[device.getSerial()] =
                                    setTimeout(() => {
                                        device.updateProperty(
                                            PropertyName.DeviceSnooze,
                                            false
                                        ...
                                        delete this.deviceSnoozeTimeout[
                                            device.getSerial()
                                        ];
                                    }, timeoutMS);
                            })
            ...
            if (
                result.customData !== undefined &&
                result.customData.onFailure !== undefined
            ) {
                try {
                    ...
                    const error = ensureError(err);
                    rootMainLogger.error(
                        `Station command result - onFailure callback error`,
                        {
                            error: getError(error),
                            stationSN: station.getSerial(),
                            result: result,
            ...
            this.getStationDevice(station.getSerial(), result.channel)
                ...
                .catch((err) => {
                    ...
                    if (error instanceof DeviceNotFoundError) {
                        ...
                        rootMainLogger.error(
                            `Station secondary command result error`,
                            {
                                error: getError(error),
                                stationSN: station.getSerial(),
                                result: result,
    ...
    private onDevicePropertyChanged(
        ...
        try {
            ...
            if (
                ...
                (device.getPropertyValue(PropertyName.DeviceRTSPStreamUrl) ===
                    undefined ||
                ...
                this.getStation(device.getStationSerial())
                    ...
                    .catch((err) => {
                        const error = ensureError(err);
                        rootMainLogger.error(
                            `Device property changed error - station enable rtsp`,
                            {
                                error: getError(error),
                                deviceSN: device.getSerial(),
                ...
                if (!isValidUrl(value as string)) {
                    this.getStation(device.getStationSerial())
                        ...
                        .catch((err) => {
                            const error = ensureError(err);
                            rootMainLogger.error(
                                `Device property changed error - station download image`,
                                {
                                    error: getError(error),
                                    deviceSN: device.getSerial(),
                                    stationSN: device.getStationSerial(),
    ...
    private onDeviceReady(device: Device): void {
        try {
            if (
                device.getPropertyValue(PropertyName.DeviceRTSPStream) !==
                    undefined &&
                (device.getPropertyValue(
                ...
                this.getStation(device.getStationSerial())
                    ...
                    .catch((err) => {
                        const error = ensureError(err);
                        rootMainLogger.error(
                            `Device ready error - station enable rtsp`,
                            {
                                error: getError(error),
                                deviceSN: device.getSerial(),
                                stationSN: device.getStationSerial(),
        ...
        this.getStationDevice(station.getSerial(), channel)
            .then((device: Device) => {
                if (device.hasProperty(PropertyName.DeviceBattery)) {
                    const metadataBattery = device.getPropertyMetadata(
                        PropertyName.DeviceBattery
                    );
                ...
                if (device.hasProperty(PropertyName.DeviceBatteryTemp)) {
                    const metadataBatteryTemperature =
                        device.getPropertyMetadata(
        ...
        this.getStationDevice(station.getSerial(), channel)
            .then((device: Device) => {
                ...
                if (device.hasProperty(PropertyName.DeviceChargingStatus)) {
                    const metadataChargingStatus = device.getPropertyMetadata(
        ...
        this.getStationDevice(station.getSerial(), channel)
            .then((device: Device) => {
                if (device.hasProperty(PropertyName.DeviceWifiRSSI)) {
                    const metadataWifiRssi = device.getPropertyMetadata(
        ...
        this.getStationDevice(station.getSerial(), channel)
            .then((device: Device) => {
                if (device.hasProperty(PropertyName.DeviceLight)) {
                    const metadataLight = device.getPropertyMetadata(
        ...
        if (!device.hasCommand(CommandName.DeviceStartTalkback))
            throw new NotSupportedError(
                "This functionality is not implemented or supported by this device",
                {
                    context: {
                        device: deviceSN,
                        commandName: CommandName.DeviceStartTalkback,
                    },
                }
            );
        ...
        if (!device.hasCommand(CommandName.DeviceStopTalkback))
            throw new NotSupportedError(
                "This functionality is not implemented or supported by this device",
                {
                    context: {
                        device: deviceSN,
                        commandName: CommandName.DeviceStopTalkback,
                    },
                }
            );
        ...
        this.getDevice(deviceSN)
            .then((device: Device) => {
                if (device.isSmartSafe())
                    (device as SmartSafe).shakeEvent(
                        event,
                        this.config.eventDurationSeconds
                    );
            })
    ...
    private onStationDevice911Alarm(
        ...
        this.getDevice(deviceSN)
            .then((device: Device) => {
                if (device.isSmartSafe())
                    (device as SmartSafe).alarm911Event(
                        event,
                        this.config.eventDurationSeconds
                    );
            })
    ...
    private onStationDeviceJammed(deviceSN: string): void {
        this.getDevice(deviceSN)
            .then((device: Device) => {
                if (device.isSmartSafe())
                    (device as SmartSafe).jammedEvent(
                        this.config.eventDurationSeconds
                    );
            })
            .catch((err) => {
                const error = ensureError(err);
                rootMainLogger.error(`onStationDeviceJammed error`, {
                    error: getError(error),
                    deviceSN: deviceSN,
                });
            });
    ...
    private onStationDeviceLowBattery(deviceSN: string): void {
        this.getDevice(deviceSN)
            .then((device: Device) => {
                if (device.isSmartSafe())
                    (device as SmartSafe).lowBatteryEvent(
                        this.config.eventDurationSeconds
                    );
            })
            .catch((err) => {
                const error = ensureError(err);
                rootMainLogger.error(`onStationDeviceLowBattery error`, {
                    error: getError(error),
                    deviceSN: deviceSN,
                });
            });
    ...
    private onStationDeviceWrongTryProtectAlarm(deviceSN: string): void {
        this.getDevice(deviceSN)
            .then((device: Device) => {
                if (device.isSmartSafe())
                    (device as SmartSafe).wrongTryProtectAlarmEvent(
                        this.config.eventDurationSeconds
                    );
            })
            .catch((err) => {
                const error = ensureError(err);
                rootMainLogger.error(
                    `onStationDeviceWrongTryProtectAlarm error`,
                    {
                        error: getError(error),
                        deviceSN: deviceSN,
                    }
    ...
    public async addUser(
        ...
        try {
            ...
            const addUserResponse = await this.api.addUser(
                deviceSN,
                username,
                device.getStationSerial()
            );
    ...
    public async deleteUser(deviceSN: string, username: string): Promise<void> {
        ...
        if (!device.hasCommand(CommandName.DeviceDeleteUser))
            throw new NotSupportedError(
                "This functionality is not implemented or supported by this device",
                {
                    context: {
                        device: deviceSN,
                        commandName: CommandName.DeviceDeleteUser,
                        username: username,
                    },
                }
            );
        ...
        try {
            ...
            const error = ensureError(err);
            rootMainLogger.error(`deleteUser error`, {
                error: getError(error),
                deviceSN: deviceSN,
                username: username,
            });
                    ...
                    if (user.user_name === username) {
                        if (
                            ...
                            for (const entry of user.password_list) {
                                if (
                                    entry.password_type === UserPasswordType.PIN
                                ) {
    ...
    public async updateUserPasscode(
        deviceSN: string,
        username: string,
        passcode: string
    ): Promise<void> {
    ...
    private _emitStationImageDownload(
        ...
        this.getDevicesFromStation(station.getSerial())
            ...
            .catch((err) => {
                const error = ensureError(err);
                rootMainLogger.error(
                    `onStationImageDownload - Set picture error`,
                    {
                        error: getError(error),
                        stationSN: station.getSerial(),
                        file: file,
        ...
        this.getStationDevice(station.getSerial(), channel)
            .then((device: Device) => {
                if (device.hasProperty(PropertyName.DeviceSensorOpen)) {
                    const metadataSensorOpen = device.getPropertyMetadata(
