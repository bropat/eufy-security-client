...
export class EufySecurity extends TypedEmitter<EufySecurityEvents> {
    ...
    public async connectToStation(
        ...
        if (station.isP2PConnectableDevice()) {
            ...
            rootMainLogger.debug(`Explicit request for p2p connection to the station`, {
                ...
                promises.push(
                    station.then((station: Station) => {
                        try {
                            ...
                            station.on(
                                "property changed",
                                (station: Station, name: string, value: PropertyValue, ready: boolean) =>
                            ...
                            station.on(
                                "talkback started",
                                (station: Station, channel: number, talkbackStream: TalkbackStream) =>
                            ...
                            station.on(
                                "garage door status",
                                (station: Station, channel: number, doorId: number, status: number) =>
                                    this.onStationGarageDoorStatus(station, channel, doorId, status)
                            );
                            station.on(
                                "storage info hb3",
                                (station: Station, channel: number, storageInfo: StorageInfoBodyHB3) =>
