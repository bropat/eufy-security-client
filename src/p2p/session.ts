...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
    ...
    public async sendPing(address: Address): Promise<void> {
        if (
            (this.lastPong &&
                (new Date().getTime() - this.lastPong) / this.getHeartbeatInterval() >= this.MAX_RETRIES) ||
            (this.connectTime &&
                !this.lastPong &&
        ...
        if (messageState) {
            ...
            messageState.retryTimeout = setTimeout(() => {
                if (this.connectAddress) {
                    sendMessage(this.socket, this.connectAddress, RequestMessageType.DATA, messageState.data).catch(
                        (err) => {
                            const error = ensureError(err);
    ...
    private async _sendCommand(message: P2PMessageState | P2PQueueMessage): Promise<void> {
        if (isP2PQueueMessage(message)) {
            ...
            if (message.retries < this.MAX_RETRIES && message.returnCode !== ErrorCode.ERROR_CONNECT_TIMEOUT) {
                ...
                this.emit("command", {
                    command_type:
                        message.nestedCommandType !== undefined ? message.nestedCommandType : message.commandType,
                    channel: message.channel,
            ...
            if (!this.connected) {
                ...
                if (p2pDid === this.rawStation.p2p_did) {
                    rootP2PLogger.debug(
                        ...
                        { stationSN: this.rawStation.station_sn, ip: rinfo.address, port: rinfo.port, p2pDid: p2pDid }
                    );
                            ...
                            if (
                                ...
                                if (msg_state.customData) {
                                    for (const index of Object.keys(this.customDataStaging)) {
                                        ...
                                        if (
                                            new Date().getTime() - this.customDataStaging[id].timestamp >
                                            2 * 60 * 1000
                                        ) {
        ...
        if (!this.currentMessageState[message.dataType].invalidStream) {
            switch (message.commandId) {
                case CommandType.CMD_VIDEO_FRAME:
                        ...
                        if (rsaKey) {
                            try {
                                ...
                                rootP2PLogger.trace(
                                    ...
                                    { stationSN: this.rawStation.station_sn, key: videoMetaData.aesKey }
                                );
                            } catch (err) {
                                const error = ensureError(err);
                                rootP2PLogger.warn(
                    ...
                    this.currentMessageState[message.dataType].p2pStreamMetadata.videoHeight =
                            ...
                            rootP2PLogger.trace(
                                `Handle DATA ${P2PDataType[message.dataType]} - CMD_VIDEO_FRAME: startcode found`,
                                {
                                    ...
                                    preFrameVideoDataLength:
                                        this.currentMessageState[message.dataType].preFrameVideoData.length,
                                }
                            );
                ...
                case CommandType.SUB1G_REP_UNPLUG_POWER_LINE:
                    try {
                        rootP2PLogger.debug(
                            ...
                            { stationSN: this.rawStation.station_sn, payload: data.toString() }
                        );
                ...
                case CommandType.CMD_SET_FLOODLIGHT_MANUAL_SWITCH:
                    try {
                        ...
                        rootP2PLogger.debug(
                            ...
                            { stationSN: this.rawStation.station_sn, enabled: enabled, payload: data.toString() }
                        );
                                ...
                                for (const record of data) {
                                    for (const tableRecord of record.payload) {
                                        ...
                                        if (record.table_name === "history_record_info") {
                                            tmpRecord.history = {
                                                device_type: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .device_type,
                                                account: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .account,
                                                start_time: parse(
                                                ...
                                                frame_num: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .frame_num,
                                                storage_type: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .storage_type,
                                                storage_cloud: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .storage_cloud,
                                                cipher_id: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .cipher_id,
                                                vision: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo).vision,
                                                video_type: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .video_type,
                                                has_lock: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .has_lock,
                                                automation_id: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .automation_id,
                                                trigger_type: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .trigger_type,
                                                push_mode: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .push_mode,
                                                mic_status: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .mic_status,
                                                res_change: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .res_change,
                                                res_best_width: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .res_best_width,
                                                res_best_height: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .res_best_height,
                                                self_learning: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .self_learning,
                                                storage_path: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .storage_path,
                                                thumb_path: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .thumb_path,
                                                write_status: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .write_status,
                                                cloud_path: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .cloud_path,
                                                folder_size: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .folder_size,
                                                storage_status: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .storage_status,
                                                storage_label: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .storage_label,
                                                time_zone: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .time_zone,
                                                mp4_cloud: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .mp4_cloud,
                                                snapshot_cloud: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .snapshot_cloud,
                                                table_version: (tableRecord as P2PDatabaseQueryLocalHistoryRecordInfo)
                                                    .table_version,
                                            };
                                            ...
                                            tmpRecord.picture.push({
                                                picture_id: (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                                                    .picture_id,
                                                detection_type: (
                                                    tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo
                                                ).detection_type,
                                                person_id: (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                                                    .person_id,
                                                crop_path: (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                                                    .crop_path,
                                                event_time: parse(
                                                    (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                                                        .event_time,
                                                ...
                                                group_id: (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                                                    .group_id,
                                                crop_id: (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                                                    .crop_id,
                                                start_time: parse(
                                                    (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                                                        .start_time,
                                                ...
                                                update_time: (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                                                    .update_time,
                                            });
                                        } else {
                                            rootP2PLogger.debug(
