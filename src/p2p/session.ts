...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
    ...
    private messageStates: SortedMap<number, P2PMessageState> = new SortedMap<
        number,
        P2PMessageState
    >((a: number, b: number) => a - b);
    private messageVideoStates: SortedMap<number, P2PVideoMessageState> =
        new SortedMap<number, P2PVideoMessageState>(
            (a: number, b: number) => a - b
        );
    ...
    private _wasSequenceNumberAlreadyProcessed(
        ...
        if (expectedSequence - this.SEQUENCE_PROCESSING_BOUNDARY > 0) {
            ...
            return this._isBetween(
                receivedSequence,
                expectedSequence - this.SEQUENCE_PROCESSING_BOUNDARY,
                expectedSequence
            );
        } else {
            // there was a sequence number reset recently
            const isInRangeAfterReset = this._isBetween(
                receivedSequence,
                0,
                expectedSequence
            );
    ...
    private initializeMessageState(
        ...
        rsaKey: NodeRSA | null = null
    ): void {
        this.currentMessageState[datatype] = {
            leftoverData: Buffer.from([]),
            queuedData: new SortedMap<number, P2PMessage>(
                (a: number, b: number) => a - b
            ),
            rsaKey: rsaKey,
            videoStream: null,
    ...
    private _disconnected(): void {
        ...
        for (const channel in this.currentMessageState[P2PDataType.DATA]
            .rtspStreaming) {
    ...
    private closeEnergySavingDevice(): void {
        if (
            this.sendQueue.filter(
                (queue) =>
                    queue.p2pCommand.commandType !== CommandType.CMD_PING &&
                    queue.p2pCommand.commandType !==
                        CommandType.CMD_GET_DEVICE_PING
            ).length === 0 &&
            this.energySavingDevice &&
            !this.isCurrentlyStreaming() &&
            Array.from(this.messageStates.values()).filter(
                (msgState) => msgState.acknowledged === false
            ).length === 0
        ) {
            if (this.esdDisconnectTimeout === undefined) {
                ...
                this.esdDisconnectTimeout = setTimeout(() => {
                    this.esdDisconnectTimeout = undefined;
                    this.sendMessage(
                        `Closing of connection for battery saving`,
                        this.connectAddress!,
                        RequestMessageType.END
                    );
                    rootP2PLogger.info(
    ...
    private async renewDSKKey(): Promise<void> {
        if (
            ...
            rootP2PLogger.debug(
                `DSK keys not present or expired, get/renew it`,
                {
                    stationSN: this.rawStation.station_sn,
                    dskKey: this.dskKey,
    ...
    private localLookup(host: string): void {
        rootP2PLogger.debug(
            `Trying to local lookup address for station ${this.rawStation.station_sn} with host ${host}`
        );
    ...
    private cloudLookup(): void {
        this.cloudAddresses.map((address) =>
            this.cloudLookupByAddress(address)
        );
        this._clearLookupRetryTimeout();
        this.lookupRetryTimeout = setTimeout(() => {
            this.cloudLookup();
    ...
    private cloudLookup2(): void {
        this.cloudAddresses.map((address) =>
            this.cloudLookupByAddress2(address)
        );
    ...
    private cloudLookupWithTurnServer(
        ...
        this.cloudAddresses.map((address) =>
            this.cloudLookupByAddressWithTurnServer(address, origAddress, data)
        );
    }
    ...
    private async cloudLookupByAddress(address: Address): Promise<void> {
        ...
        const payload = buildLookupWithKeyPayload(
            this.socket,
            this.rawStation.p2p_did,
            this.dskKey
        );
        await this.sendMessage(
            `Cloud lookup addresses`,
            address,
            msgId,
            payload
        );
    ...
    private async cloudLookupByAddress2(address: Address): Promise<void> {
        ...
        const payload = buildLookupWithKeyPayload2(
            this.rawStation.p2p_did,
            this.dskKey
        );
        await this.sendMessage(
            `Cloud lookup addresses (2)`,
            address,
            msgId,
            payload
        );
    ...
    private async cloudLookupByAddressWithTurnServer(
        ...
        const payload = buildLookupWithKeyPayload3(
            this.rawStation.p2p_did,
            origAddress,
            data
        );
        await this.sendMessage(
            `Cloud lookup addresses with turn server`,
            address,
            msgId,
            payload
        );
    ...
    private _connect(address: Address, p2p_did: string): void {
        rootP2PLogger.debug(
            `Connecting to host ${address.host} on port ${address.port} (CHECK_CAM)`,
            {
                stationSN: this.rawStation.station_sn,
                address: address,
                p2pDid: p2p_did,
    ...
    private lookup(host?: string): void {
        ...
        this.lookupTimeout = setTimeout(() => {
            this.lookupTimeout = undefined;
            rootP2PLogger.error(`All address lookup tentatives failed.`, {
                stationSN: this.rawStation.station_sn,
            });
    ...
    public async connect(host?: string): Promise<void> {
        if (
            !this.connected &&
            !this.connecting &&
            this.rawStation.p2p_did !== undefined
        ) {
            ...
            if (!this.binded)
                this.socket.bind(this.listeningPort, () => {
                    this.binded = true;
                    try {
                        this.socket.setRecvBufferSize(
                            this.UDP_RECVBUFFERSIZE_BYTES
                        );
                        ...
                        rootP2PLogger.error(`connect - Error`, {
                            ...
                            stationSN: this.rawStation.station_sn,
                            host: host,
                            currentRecBufferSize:
                                this.socket.getRecvBufferSize(),
                            recBufferRequestedSize:
                                this.UDP_RECVBUFFERSIZE_BYTES,
                        });
                    }
                    this.lookup(host);
    ...
    public async sendPing(address: Address): Promise<void> {
        if (
            ...
            (this.connectTime &&
                !this.lastPong &&
                (new Date().getTime() - this.connectTime) /
                    this.getHeartbeatInterval() >=
                    this.MAX_RETRIES)
        ) {
    ...
    public sendCommandWithIntString(
        ...
        this.sendCommand(
            p2pcommand,
            InternalP2PCommandType.WithIntString,
            undefined,
            customData
        );
    }

    public sendCommandWithInt(
        p2pcommand: P2PCommand,
        customData?: CustomData
    ): void {
        ...
        this.sendCommand(
            p2pcommand,
            InternalP2PCommandType.WithInt,
            undefined,
            customData
        );
    }

    public sendCommandWithStringPayload(
        p2pcommand: P2PCommand,
        customData?: CustomData
    ): void {
        if (p2pcommand.channel === undefined) p2pcommand.channel = 0;
        ...
        rootP2PLogger.debug(`sendCommandWithStringPayload:`, {
            p2pcommand: p2pcommand,
            customData: customData,
        });
        ...
        if (p2pcommand.commandType == CommandType.CMD_SET_PAYLOAD) {
            try {
                ...
                const error = ensureError(err);
                rootP2PLogger.error(
                    `sendCommandWithStringPayload CMD_SET_PAYLOAD - Error`,
                    {
                        error: getError(error),
                        stationSN: this.rawStation.station_sn,
                        p2pcommand: p2pcommand,
            ...
            p2pcommand.commandType == CommandType.CMD_DOORBELL_SET_PAYLOAD
        ) {
            try {
                const json = JSON.parse(p2pcommand.value);
                ...
                const error = ensureError(err);
                rootP2PLogger.error(
                    `sendCommandWithStringPayload CMD_DOORBELL_SET_PAYLOAD - Error`,
                    {
                        error: getError(error),
                        stationSN: this.rawStation.station_sn,
                        p2pcommand: p2pcommand,
    ...
    public sendCommandWithString(
        ...
        this.sendCommand(
            p2pcommand,
            InternalP2PCommandType.WithString,
            p2pcommand.commandType,
            customData
        );
    }
    ...
    private sendQueuedMessage(): void {
        if (this.sendQueue.length > 0) {
            if (this.connected) {
                ...
                while (
                    (queuedMessage = this.sendQueue.shift()!) !== undefined
                ) {
                    let exists = false;
                    let waitingAcknowledge = false;
                    this.messageStates.forEach((stateMessage) => {
                        if (
                            stateMessage.commandType ===
                                queuedMessage.p2pCommand.commandType &&
                            stateMessage.nestedCommandType ===
                                queuedMessage.nestedCommandType &&
                ...
                rootP2PLogger.debug(
                    `Initiate station p2p connection to send queued data`,
                    {
                        stationSN: this.rawStation.station_sn,
    ...
    private sendCommand(
        ...
        if (
            p2pcommand.commandType !== CommandType.CMD_PING &&
            p2pcommand.commandType !== CommandType.CMD_GET_DEVICE_PING
        )
            this._clearESDDisconnectTimeout();
            ...
            messageState.retryTimeout = setTimeout(() => {
                if (this.connectAddress) {
                    sendMessage(
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `resendNotAcknowledgedCommand - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                sequence: sequence,
    ...
    private async _sendCommand(
        message: P2PMessageState | P2PQueueMessage
    ): Promise<void> {
        if (isP2PQueueMessage(message)) {
            const ageing = +new Date() - message.timestamp;
            if (ageing <= this.MAX_COMMAND_QUEUE_TIMEOUT) {
                const commandHeader = buildCommandHeader(
                    this.seqNumber,
                    message.p2pCommand.commandType
                );
                let payload: Buffer;
                const channel =
                    message.p2pCommand.channel !== undefined
                        ? message.p2pCommand.channel
                        : 0;
                switch (message.p2pCommandType) {
                    ...
                    case InternalP2PCommandType.WithIntString:
                        payload = buildIntStringCommandPayload(
                            ...
                            message.p2pCommand.commandType,
                            message.p2pCommand.value as number,
                            message.p2pCommand.valueSub === undefined
                                ? 0
                ...
                rootP2PLogger.warn(
                    `Command aged out from send queue for station ${this.rawStation.station_sn}`,
                    {
                        commandType: message.p2pCommand.commandType,
                        nestedCommandType: message.nestedCommandType,
            ...
            if (
                message.retries < this.MAX_RETRIES &&
                message.returnCode !== ErrorCode.ERROR_CONNECT_TIMEOUT
            ) {
                ...
                if (
                    ...
                    if (
                        this.currentMessageState[P2PDataType.VIDEO]
                            .p2pStreaming &&
                        message.channel ===
                            this.currentMessageState[P2PDataType.VIDEO]
                                .p2pStreamChannel
                    ...
                    message.commandType === CommandType.CMD_DOWNLOAD_VIDEO
                ) {
                    if (
                        this.currentMessageState[P2PDataType.BINARY]
                            .p2pStreaming &&
                        message.channel ===
                            this.currentMessageState[P2PDataType.BINARY]
                                .p2pStreamChannel
        ...
        if (messageState.commandType !== CommandType.CMD_PING) {
            ...
            this.p2pDataSeqNumber = this._incrementSequence(
        ...
        if (messageState.retries === 0) {
            if (
                messageState.commandType ===
                    CommandType.CMD_START_REALTIME_MEDIA ||
                (messageState.nestedCommandType ===
                    CommandType.CMD_START_REALTIME_MEDIA &&
                ...
                this.currentMessageState[P2PDataType.VIDEO].p2pStreamChannel =
                    messageState.channel;
                ...
                this.currentMessageState[P2PDataType.BINARY].p2pStreaming =
                    true;
                this.currentMessageState[P2PDataType.BINARY].p2pStreamChannel =
                    message.channel;
                this.waitForStreamData(P2PDataType.BINARY);
            } else if (
                messageState.commandType === CommandType.CMD_STOP_REALTIME_MEDIA
            ) {
                //TODO: CommandType.CMD_RECORD_PLAY_CTRL only if stop
                this.endStream(P2PDataType.VIDEO);
            } else if (
                messageState.commandType === CommandType.CMD_DOWNLOAD_CANCEL
            ) {
                this.endStream(P2PDataType.BINARY);
            } else if (messageState.commandType === CommandType.CMD_NAS_TEST) {
                if (
                    this.currentMessageState[P2PDataType.DATA].rtspStream[
                        messageState.channel
                    ]
                ) {
        ...
        if (hasHeader(msg, ResponseMessageType.LOCAL_LOOKUP_RESP)) {
            if (!this.connected) {
                ...
                if (p2pDid === this.rawStation.p2p_did) {
                    rootP2PLogger.debug(
                        `Received message - LOCAL_LOOKUP_RESP - Wanted device was found, connect to it`,
                        {
                            stationSN: this.rawStation.station_sn,
                            ip: rinfo.address,
                            port: rinfo.port,
                    ...
                    rootP2PLogger.debug(
                        `Received message - LOCAL_LOOKUP_RESP - Unwanted device was found, don't connect to it`,
                        {
                            stationSN: this.rawStation.station_sn,
                            ip: rinfo.address,
                            port: rinfo.port,
            ...
            if (!this.connected) {
                ...
                rootP2PLogger.trace(
                    `Received message - LOOKUP_ADDR - Got response`,
                    {
                        stationSN: this.rawStation.station_sn,
                ...
                if (this.connectionType === P2PConnectionType.ONLY_LOCAL) {
                    if (isPrivateIp(ip)) {
                        ...
                        this._clearLookupRetryTimeout();
                        rootP2PLogger.debug(
                            `Trying to connect in ONLY_LOCAL mode...`,
                            {
                                stationSN: this.rawStation.station_sn,
                                ip: ip,
                                port: port,
                    ...
                    this._clearLookupRetryTimeout();
                    rootP2PLogger.debug(
                        `Trying to connect in QUICKEST mode...`,
                        {
                            stationSN: this.rawStation.station_sn,
                            ip: ip,
                            port: port,
            ...
            hasHeader(msg, ResponseMessageType.CAM_ID) ||
            hasHeader(msg, ResponseMessageType.TURN_SERVER_CAM_ID)
        ) {
            ...
            if (!this.connected) {
                ...
                if (Device.isSmartSafe(this.rawStation.device_type)) {
                    ...
                    const tmpSendQueue: Array<P2PQueueMessage> = [
                        ...this.sendQueue,
                    ];
                    this.sendQueue = [];
            ...
            for (let i = 1; i <= numAcks; i++) {
                ...
                rootP2PLogger.trace(
                    `Received message - ACK ${P2PDataType[dataType]} - sequence ${ackedSeqNo}`,
                    {
                        stationSN: this.rawStation.station_sn,
                        remoteAddress: rinfo.address,
                        remotePort: rinfo.port,
                            ...
                            msg_state.timeout = setTimeout(
                                () => {
                                    ...
                                    if (
                                        ...
                                        rootP2PLogger.warn(
                                            `Result data for command not received`,
                                            {
                                                stationSN:
                                                    this.rawStation.station_sn,
                                                message: {
                                                    ...
                                                    acknowledged:
                                                        msg_state.acknowledged,
                                                    retries: msg_state.retries,
                                                    returnCode:
                                                        msg_state.returnCode,
                                                    data: msg_state.data,
                                                },
                                            }
                                        );
                                        ...
                                        this.p2pDataSeqNumber--;
                                        rootP2PLogger.debug(
                                            `Result data for command CMD_GATEWAYINFO not received`,
                                            {
                                                stationSN:
                                                    this.rawStation.station_sn,
                                                message: {
            ...
            if (this.connected) {
                ...
                if (message.seqNo === this.expectedSeqNo[dataType]) {
                    ...
                    const timeout =
                        this.currentMessageState[dataType].waitForSeqNoTimeout;
                    if (!!timeout) {
                        clearTimeout(timeout);
                        this.currentMessageState[dataType].waitForSeqNoTimeout =
                            undefined;
                    ...
                    rootP2PLogger.trace(
                        `Received message - DATA ${P2PDataType[message.type]} - Received expected sequence`,
                        {
                            stationSN: this.rawStation.station_sn,
                            remoteAddress: rinfo.address,
                            remotePort: rinfo.port,
                            ...
                            queuedDataSize:
                                this.currentMessageState[dataType].queuedData
                                    .size,
                        }
                    );

                    let queuedMessage = this.currentMessageState[
                        dataType
                    ].queuedData.get(this.expectedSeqNo[dataType]);
                    while (queuedMessage) {
                        rootP2PLogger.trace(
                            `Received message - DATA ${P2PDataType[queuedMessage.type]} - Work off queued data`,
                            {
                                stationSN: this.rawStation.station_sn,
                                remoteAddress: rinfo.address,
                                remotePort: rinfo.port,
                        ...
                        this.currentMessageState[dataType].queuedData.delete(
                            queuedMessage.seqNo
                        );
                        queuedMessage = this.currentMessageState[
                            dataType
                        ].queuedData.get(this.expectedSeqNo[dataType]);
                    }
                } else if (
                    this._wasSequenceNumberAlreadyProcessed(
                        this.expectedSeqNo[dataType],
                        message.seqNo
                    )
                ) {
                    ...
                    if (!this.currentMessageState[dataType].waitForSeqNoTimeout)
                        this.currentMessageState[dataType].waitForSeqNoTimeout =
                            setTimeout(() => {
                                ...
                                this.currentMessageState[
                                    dataType
                                ].waitForSeqNoTimeout = undefined;
                            }, this.MAX_EXPECTED_SEQNO_WAIT);

                    if (
                        !this.currentMessageState[dataType].queuedData.get(
                            message.seqNo
                        )
                    ) {
                        this.currentMessageState[dataType].queuedData.set(
                            message.seqNo,
                            message
                        );
                        rootP2PLogger.trace(
            ...
            if (!this.connected) {
                ...
                rootP2PLogger.trace(
                    `Received message - LOOKUP_ADDR2 - Got response`,
                    {
                        stationSN: this.rawStation.station_sn,
                        remoteAddress: rinfo.address,
                        remotePort: rinfo.port,
                ...
                rootP2PLogger.debug(
                    `Connecting to host ${ip} on port ${port} (CHECK_CAM2)...`,
                    {
                        stationSN: this.rawStation.station_sn,
                        ip: ip,
                        port: port,
                ...
                if (this.p2pTurnHandshaking[ip] === undefined) {
                    ...
                    this.sendMessage(
                        `Send TURN_SERVER_INIT`,
                        { host: ip, port: port },
                        RequestMessageType.TURN_SERVER_INIT
                    );
                }
            }
            ...
            if (!this.connected && !this.p2pTurnConfirmed) {
                rootP2PLogger.trace(
                    `Received message - TURN_SERVER_OK - Got response`,
                    {
                        stationSN: this.rawStation.station_sn,
                        remoteAddress: rinfo.address,
                        remotePort: rinfo.port,
            ...
            if (!this.connected) {
                ...
                for (let i = 0; i < 4; i++)
                    this.sendCamCheck2(
                        { host: rinfo.address, port: port },
                        binaryIP
                    );

                rootP2PLogger.trace(
                    `Received message - TURN_SERVER_TOKEN - Got response`,
                    {
                        stationSN: this.rawStation.station_sn,
                        remoteAddress: rinfo.address,
                        remotePort: rinfo.port,
                ...
                this.cloudLookupWithTurnServer(
                    { host: rinfo.address, port: port },
                    binaryIP
                );
            }
            ...
            if (!this.connected) {
                ...
                rootP2PLogger.trace(
                    `Received message - LOOKUP_RESP - Got response`,
                    {
                        stationSN: this.rawStation.station_sn,
                        remoteAddress: rinfo.address,
                        remotePort: rinfo.port,
    ...
    private parseDataMessage(message: P2PMessage): void {
        if (
            (message.type === P2PDataType.BINARY ||
                message.type === P2PDataType.VIDEO) &&
            !this.currentMessageState[message.type].p2pStreaming
        ) {
            rootP2PLogger.trace(
                `Parsing message - DATA ${P2PDataType[message.type]} - Stream not started ignore this data`,
                {
                    stationSN: this.rawStation.station_sn,
                    seqNo: message.seqNo,
            ...
            if (
                this.currentMessageState[message.type].leftoverData.length > 0
            ) {
                message.data = Buffer.concat([
                    this.currentMessageState[message.type].leftoverData,
                    message.data,
                ]);
                this.currentMessageState[message.type].leftoverData =
                    Buffer.from([]);
            ...
            do {
                ...
                const firstPartMessage =
                    data.subarray(0, 4).toString() === MAGIC_WORD;
                ...
                if (firstPartMessage) {
                    ...
                    if (data.length >= header.bytesToRead) {
                        ...
                        this.currentMessageBuilder[message.type].messages[
                            message.seqNo
                        ] = payload;
                        this.currentMessageBuilder[message.type].bytesRead =
                            payload.byteLength;
                    ...
                    if (
                        this.currentMessageBuilder[message.type].header
                            .bytesToRead -
                            this.currentMessageBuilder[message.type]
                                .bytesRead ===
                            0 &&
                ...
                rootP2PLogger.trace(
                    `Parsing message - DATA ${P2PDataType[message.type]} - Received data`,
                    {
                        stationSN: this.rawStation.station_sn,
                        seqNo: message.seqNo,
                        header: this.currentMessageBuilder[message.type].header,
                ...
                if (
                    ...
                    const completeMessage = sortP2PMessageParts(
                        this.currentMessageBuilder[message.type].messages
                    );
                    ...
                    if (data.length > 0 && message.type === P2PDataType.DATA) {
                        rootP2PLogger.debug(
                            `Parsing message - DATA ${P2PDataType[message.type]} - Parsed data`,
                            {
                                stationSN: this.rawStation.station_sn,
                                seqNo: message.seqNo,
                ...
                data.length > 0 &&
                runaway_limit < this.LOOP_RUNAWAY_LIMIT
            );
        ...
        if (message.dataType === P2PDataType.CONTROL) {
            ...
            if (result_msg) {
                ...
                if (message.bytesToRead > 0) {
                    if (message.signCode > 0) {
                        try {
                            ...
                            const error = ensureError(err);
                            rootP2PLogger.debug(
                                `Handle DATA ${P2PDataType[message.dataType]} - Decrypt Error`,
                                {
                                    error: getError(error),
                                    stationSN: this.rawStation.station_sn,
                                    message: {
                    ...
                    return_code =
                        message.data.subarray(0, 4).readUInt32LE() | 0;
                    resultData = message.data.subarray(4);
                }
                const error_codeStr = ErrorCode[return_code];

                rootP2PLogger.debug(
                    `Handle DATA ${P2PDataType[message.dataType]} - Received data`,
                    {
                        stationSN: this.rawStation.station_sn,
                        seqNo: message.seqNo,
                ...
                if (msg_state) {
                    if (msg_state.commandType === message.commandId) {
                        ...
                        const command_type =
                            msg_state.nestedCommandType !== undefined
                                ...
                                : msg_state.commandType;
                        rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - Result data for command received`,
                            {
                                stationSN: this.rawStation.station_sn,
                                message: {
                        ...
                        if (return_code === ErrorCode.ERROR_FAILED_TO_REQUEST) {
                            ...
                            if (
                                ...
                                command_type ===
                                    CommandType.CMD_SET_PAYLOAD_LOCKV12 ||
                                command_type ===
                                    CommandType.CMD_TRANSFER_PAYLOAD
                            ) {
                            ...
                            if (
                                ...
                                msg_state.commandType ===
                                CommandType.CMD_DOWNLOAD_VIDEO
                            ) {
                                ...
                                const totalCapacity = resultData
                                    .subarray(0, 4)
                                    .readUInt32LE();
                                const availableCapacity = resultData
                                    .subarray(4, 8)
                                    .readUInt32LE();
                                if (return_code >= -1) {
                                    this.emit(
                    ...
                    rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - Received unexpected data!`,
                        {
                            stationSN: this.rawStation.station_sn,
                            seqNumber: this.seqNumber,
                            p2pDataSeqNumber: this.p2pDataSeqNumber,
                ...
                rootP2PLogger.debug(
                    `Handle DATA ${P2PDataType[message.dataType]} - Unsupported response`,
                    {
                        stationSN: this.rawStation.station_sn,
                        message: {
                            seqNo: message.seqNo,
            ...
            message.dataType === P2PDataType.VIDEO ||
            message.dataType === P2PDataType.BINARY
        ) {
            this.handleDataBinaryAndVideo(message);
        } else {
            rootP2PLogger.debug(
                `Handle DATA ${P2PDataType[message.dataType]} - Not implemented data type`,
                {
                    stationSN: this.rawStation.station_sn,
                    message: {
                        seqNo: message.seqNo,
    ...
    private waitForStreamData(
        ...
        if (this.currentMessageState[dataType].p2pStreamingTimeout) {
            clearTimeout(
                this.currentMessageState[dataType].p2pStreamingTimeout!
            );
        }
        ...
        if (!this.currentMessageState[message.dataType].invalidStream) {
            switch (message.commandId) {
                case CommandType.CMD_VIDEO_FRAME:
                    ...
                    const isKeyFrame =
                        message.data.subarray(4, 5).readUInt8() === 1
                            ? true
                            : false;

                    videoMetaData.videoDataLength = message.data
                        .subarray(0, 4)
                        .readUInt32LE();
                    videoMetaData.streamType = message.data
                        .subarray(5, 6)
                        .readUInt8();
                    videoMetaData.videoSeqNo = message.data
                        .subarray(6, 8)
                        .readUInt16LE();
                    videoMetaData.videoFPS = message.data
                        .subarray(8, 10)
                        .readUInt16LE();
                    videoMetaData.videoWidth = message.data
                        .subarray(10, 12)
                        .readUInt16LE();
                    videoMetaData.videoHeight = message.data
                        .subarray(12, 14)
                        .readUInt16LE();
                    videoMetaData.videoTimestamp = message.data
                        .subarray(14, 20)
                        .readUIntLE(0, 6);
                    ...
                    if (message.signCode > 0 && data_length >= 128) {
                        ...
                        const rsaKey =
                            this.currentMessageState[message.dataType].rsaKey;
                        if (rsaKey) {
                            try {
                                ...
                                const error = ensureError(err);
                                rootP2PLogger.warn(
                                    `Error: AES key could not be decrypted! The entire stream is discarded.`,
                                    {
                                        error: getError(error),
                                        stationSN: this.rawStation.station_sn,
                    ...
                    if (videoMetaData.aesKey !== '') {
                        ...
                        video_data = message.data.subarray(
                            payloadStart,
                            payloadStart + videoMetaData.videoDataLength
                        );
                    }
                    ...
                    rootP2PLogger.trace(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_VIDEO_FRAME`,
                        {
                            stationSN: this.rawStation.station_sn,
                    ...
                    this.currentMessageState[
                        message.dataType
                    ].p2pStreamMetadata.videoFPS = videoMetaData.videoFPS;
                    this.currentMessageState[
                        message.dataType
                    ].p2pStreamMetadata.videoHeight = videoMetaData.videoHeight;
                    this.currentMessageState[
                        message.dataType
                    ].p2pStreamMetadata.videoWidth = videoMetaData.videoWidth;

                    if (
                        !this.currentMessageState[message.dataType]
                        ...
                        this.currentMessageState[
                            message.dataType
                        ].p2pStreamFirstVideoDataReceived = true;
                        if (
                            !this.currentMessageState[message.dataType]
                                .p2pStreamFirstAudioDataReceived
                        ) {
                            this.currentMessageState[
                                message.dataType
                            ].waitForAudioData = setTimeout(() => {
                                this.currentMessageState[
                                    message.dataType
                                ].waitForAudioData = undefined;
                                this.currentMessageState[
                                    message.dataType
                                ].p2pStreamMetadata.audioCodec =
                                    AudioCodec.NONE;
                                this.currentMessageState[
                                    message.dataType
                                ].p2pStreamFirstAudioDataReceived = true;
                                if (
                                    this.currentMessageState[message.dataType]
                                        .p2pStreamFirstAudioDataReceived &&
                                    this.currentMessageState[message.dataType]
                    ...
                    if (message.dataType === P2PDataType.VIDEO) {
                        if (findStartCode(video_data)) {
                            rootP2PLogger.trace(
                                `Handle DATA ${P2PDataType[message.dataType]} - CMD_VIDEO_FRAME: startcode found`,
                                {
                                    stationSN: this.rawStation.station_sn,
                                    isKeyFrame: isKeyFrame,
                                    preFrameVideoDataLength:
                                        this.currentMessageState[
                                            message.dataType
                                        ].preFrameVideoData.length,
                                }
                            );
                            if (
                                !this.currentMessageState[message.dataType]
                                ...
                                this.currentMessageState[
                                    message.dataType
                                ].receivedFirstIFrame = this.isIFrame(
                                    video_data,
                                    isKeyFrame
                                );
                            ...
                            if (
                                ...
                                this.currentMessageState[
                                    message.dataType
                                ].preFrameVideoData = Buffer.from(video_data);
                            } else {
                                rootP2PLogger.trace(
                            ...
                            if (
                                ...
                                this.currentMessageState[
                                    message.dataType
                                ].preFrameVideoData = Buffer.concat([
                                    this.currentMessageState[message.dataType]
                                        .preFrameVideoData,
                                    video_data,
                                ]);
                ...
                case CommandType.CMD_AUDIO_FRAME:
                    ...
                    audioMetaData.audioDataLength = message.data
                        .subarray(0, 4)
                        .readUInt32LE();
                    audioMetaData.audioType = message.data
                        .subarray(5, 6)
                        .readUInt8();
                    audioMetaData.audioSeqNo = message.data
                        .subarray(6, 8)
                        .readUInt16LE();
                    audioMetaData.audioTimestamp = message.data
                        .subarray(8, 14)
                        .readUIntLE(0, 6);

                    const audio_data = Buffer.from(message.data.subarray(16));
                    rootP2PLogger.trace(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_AUDIO_FRAME`,
                        {
                            stationSN: this.rawStation.station_sn,
                            dataSize: message.data.length,
                    ...
                    this.currentMessageState[
                        message.dataType
                    ].audioStream?.push(audio_data);
                    break;
                default:
                    rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - Not implemented message`,
                        {
                            stationSN: this.rawStation.station_sn,
        ...
        try {
            ...
            switch (message.commandId) {
                    ...
                    try {
                        ...
                        this.emit(
                            ...
                            parseJSON(
                                cameraData,
                                rootP2PLogger
                            ) as CmdCameraInfoResponse
                        );
                    } catch (err) {
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - Camera info - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_CONVERT_MP4_OK:
                    ...
                    rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_CONVERT_MP4_OK`,
                        {
                            stationSN: this.rawStation.station_sn,
                            channel: message.channel,
                    ...
                    this.currentMessageState[
                        P2PDataType.BINARY
                    ].p2pStreamChannel = message.channel;
                    break;
                case CommandType.CMD_WIFI_CONFIG:
                    const rssi = data.readInt32LE();
                    rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_WIFI_CONFIG`,
                        {
                            stationSN: this.rawStation.station_sn,
                            channel: message.channel,
                            rssi: rssi,
                ...
                case CommandType.CMD_DOWNLOAD_FINISH:
                    rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_DOWNLOAD_FINISH`,
                        {
                            stationSN: this.rawStation.station_sn,
                            channel: message.channel,
                        }
                ...
                case CommandType.CMD_DOORBELL_NOTIFY_PAYLOAD:
                    try {
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_DOORBELL_NOTIFY_PAYLOAD - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_NAS_SWITCH:
                    try {
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_NAS_SWITCH - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.SUB1G_REP_UNPLUG_POWER_LINE:
                    try {
                        rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - SUB1G_REP_UNPLUG_POWER_LINE`,
                            {
                                stationSN: this.rawStation.station_sn,
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - SUB1G_REP_UNPLUG_POWER_LINE - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.SUB1G_REP_RUNTIME_STATE:
                    try {
                        rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - SUB1G_REP_RUNTIME_STATE`,
                            {
                                stationSN: this.rawStation.station_sn,
                                payload: data.toString(),
                            }
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - SUB1G_REP_RUNTIME_STATE - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_SET_FLOODLIGHT_MANUAL_SWITCH:
                    try {
                        const enabled =
                            data.readUIntBE(0, 1) === 1 ? true : false;
                        rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_FLOODLIGHT_MANUAL_SWITCH`,
                            {
                                stationSN: this.rawStation.station_sn,
                                enabled: enabled,
                                payload: data.toString(),
                ...
                case CommandType.CMD_GET_DEVICE_PING:
                    try {
                        rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_DEVICE_PING`,
                            {
                                stationSN: this.rawStation.station_sn,
                                payload: data.toString(),
                            }
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_DEVICE_PING - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_NOTIFY_PAYLOAD:
                    try {
                        rootP2PLogger.debug(
                            `Station Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD`,
                            {
                                stationSN: this.rawStation.station_sn,
                                payload: data.toString(),
                            }
                        ...
                        if (json !== undefined) {
                            if (
                                ...
                                if (
                                    ...
                                    json.cmd ===
                                        CommandType.P2P_GET_LOCK_PARAM ||
                                    json.cmd ===
                                    ...
                                    json.cmd ===
                                    CommandType.P2P_QUERY_STATUS_IN_LOCK
                                ) {
                                ...
                                json.cmd === CommandType.CMD_DOORLOCK_P2P_SEQ
                            ) {
                                if (
                                    Device.isLockWifi(
                                        this.rawStation.devices[0]?.device_type,
                                    ...
                                    const payload: ESLStationP2PThroughData =
                                        json.payload as ESLStationP2PThroughData;
                                    ...
                                    const payload: SmartLockP2PSequenceData =
                                        json.payload as SmartLockP2PSequenceData;
                                    if (payload.seq_num !== undefined) {
                                        rootP2PLogger.debug(
                                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD - Lock sequence number`,
                                            {
                                ...
                                json.cmd ===
                                CommandType.CMD_DOORLOCK_DATA_PASS_THROUGH
                            ) {
                                const payload: ESLStationP2PThroughData =
                                    json.payload as ESLStationP2PThroughData;
                                if (
                                    this.deviceSNs[message.channel] !==
                                    undefined
                                ) {
                                ...
                                json.cmd === CommandType.CMD_SET_PAYLOAD_LOCKV12
                            ) {
                                const payload: ESLBleV12P2PThroughData =
                                    json.payload as ESLBleV12P2PThroughData;
                                if (payload.lock_payload !== undefined) {
                                    const fac = BleCommandFactory.parseLockV12(
                                        payload.lock_payload
                                    );
                                    if (
                                        fac.getCommandCode() !==
                                        ESLBleCommand.NOTIFY
                                    ) {
                                        const aesKey = this.getLockAESKey(
                                            fac.getCommandCode()!
                                        );
                                        rootP2PLogger.debug(
                                ...
                                json.cmd === CommandType.CMD_TRANSFER_PAYLOAD
                            ) {
                                const payload: SmartLockP2PThroughData =
                                    json.payload as SmartLockP2PThroughData;
                                if (payload.lock_payload !== undefined) {
                                    try {
                                        const fac =
                                            BleCommandFactory.parseSmartLock(
                                                payload.lock_payload
                                            );
                                        rootP2PLogger.debug(
                                ...
                                Device.isSmartSafe(this.rawStation.device_type)
                            ) {
                                rootP2PLogger.debug(
                                    `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD SmartSafe`,
                                    {
                                        stationSN: this.rawStation.station_sn,
                                ...
                                switch (json.cmd) {
                                    case CommandType.CMD_SMARTSAFE_SETTINGS: {
                                        const payload =
                                            json.payload as SmartSafeSettingsNotification;
                                        try {
                                    ...
                                    case CommandType.CMD_SMARTSAFE_STATUS_UPDATE: {
                                        const payload =
                                            json.payload as SmartSafeStatusNotification;
                                        switch (payload.event_type) {
                                            case SmartSafeEvent.LOCK_STATUS: {
                                                const eventValues =
                                                    payload.event_value as SmartSafeEventValueDetail;
                                ...
                                json.cmd === CommandType.CMD_ENTRY_SENSOR_STATUS
                            ) {
                                // {"cmd":1550,"payload":{"status":1}}
                                const payload =
                                    json.payload as EntrySensorStatus;
                                rootP2PLogger.debug(
                                ...
                                const payload =
                                    json.payload as GarageDoorStatus;
                                rootP2PLogger.debug(
                                ...
                                json.cmd === CommandType.CMD_STORAGE_INFO_HB3
                            ) {
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_GET_DELAY_ALARM:
                    try {
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_DELAY_ALARM - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_SET_TONE_FILE:
                    try {
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_TONE_FILE - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_SET_SNOOZE_MODE:
                    ...
                    try {
                        rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_SNOOZE_MODE`,
                            {
                                stationSN: this.rawStation.station_sn,
                                payload: Buffer.from(
                                    data.toString(),
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_SNOOZE_MODE - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_DATABASE_IMAGE:
                    ...
                    try {
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_DATABASE_IMAGE - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_GET_TFCARD_STATUS:
                    try {
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_TFCARD_STATUS - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_DATABASE:
                    try {
                        ...
                        const databaseResponse = parseJSON(
                            str,
                            rootP2PLogger
                        ) as P2PDatabaseResponse;
                                ...
                                for (const record of data) {
                                    if (record.payload.crop_hb3_path !== '') {
                                        result.push({
                                            ...
                                            event_count:
                                                record.payload.event_count,
                                            crop_local_path:
                                                record.payload.crop_hb3_path,
                                        });
                                    } else {
                                        result.push({
                                            device_sn: record.device_sn,
                                            event_count:
                                                record.payload.event_count,
                                            crop_cloud_path:
                                                record.payload.crop_cloud_path,
                                        });
                                    }
                                }
                                this.emit(
                            ...
                            case CommandType.CMD_DATABASE_QUERY_LOCAL: {
                                ...
                                for (const record of data) {
                                    for (const tableRecord of record.payload) {
                                        let tmpRecord = result.get(
                                            tableRecord.record_id
                                        );
                                        if (tmpRecord === undefined) {
                                            tmpRecord = {
                                                record_id:
                                                    tableRecord.record_id,
                                                device_sn:
                                                    tableRecord.device_sn,
                                                station_sn:
                                                    tableRecord.station_sn,
                                            };
                                        }
                                        if (
                                            record.table_name ===
                                ...
                                this.emit(
                                    ...
                                    Array.from(
                                        result.values()
                                    ) as DatabaseQueryLocal[]
                                );
                                break;
                            }
                            case CommandType.CMD_DATABASE_DELETE: {
                                const data =
                                    databaseResponse.data as P2PDatabaseDeleteResponse;
                        ...
                        const error = ensureError(err);
                        rootP2PLogger.error(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_DATABASE - Error`,
                            {
                                error: getError(error),
                                stationSN: this.rawStation.station_sn,
                                message: {
                ...
                case CommandType.CMD_GATEWAYINFO:
                    ...
                    rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_GATEWAYINFO - cipherID`,
                        {
                            stationSN: this.rawStation.station_sn,
                            channel: message.channel,
                            data: data.toString('hex'),
                    ...
                    const encryptedKey = readNullTerminatedBuffer(
                        data.subarray(4)
                    );
                    this.api
                        .getCipher(
                            /*this.rawStation.station_sn, */ cipherID,
                        ...
                        .then((cipher) => {
                            ...
                            if (cipher !== undefined) {
                                ...
                                const rsa = getRSAPrivateKey(
                                    cipher.private_key,
                                    this.enableEmbeddedPKCS1Support
                                );
                            ...
                            this._clearTimeout(
                                this.messageStates.get(message.seqNo)?.timeout
                            );
                            this.p2pSeqMapping.forEach(
                                (
                                    value: number,
                ...
                default:
                    rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - Not implemented`,
                        {
                            stationSN: this.rawStation.station_sn,
                            commandIdName: CommandType[message.commandId],
                            commandId: message.commandId,
            ...
            const error = ensureError(err);
            rootP2PLogger.error(
                `Handle DATA ${P2PDataType[message.dataType]} - Error`,
                {
                    error: getError(error),
                    stationSN: this.rawStation.station_sn,
                    message: {
    ...
    private onError(err: any): void {
        const error = ensureError(err);
        rootP2PLogger.debug(`Socket Error:`, {
            error: getError(error),
            stationSN: this.rawStation.station_sn,
        });
    }
    ...
    private scheduleHeartbeat(): void {
        if (this.isConnected()) {
            ...
            rootP2PLogger.debug(
                `Heartbeat not activated because no connection is present!`,
                { stationSN: this.rawStation.station_sn }
            );
        }
    }
    ...
    private scheduleP2PKeepalive(): void {
        if (this.isConnected()) {
            ...
            rootP2PLogger.debug(
                `P2P keepalive not activated because no connection is present`,
                { stationSN: this.rawStation.station_sn }
            );
        }
    }
    ...
    public getDownloadRSAPrivateKey(): NodeRSA {
        if (this.currentMessageState[P2PDataType.BINARY].rsaKey === null) {
            this.currentMessageState[P2PDataType.BINARY].rsaKey =
                getNewRSAPrivateKey(this.enableEmbeddedPKCS1Support);
        }
        return this.currentMessageState[P2PDataType.BINARY].rsaKey!;
    }

    public setDownloadRSAPrivateKeyPem(pem: string): void {
        this.currentMessageState[P2PDataType.BINARY].rsaKey = getRSAPrivateKey(
            pem,
            this.enableEmbeddedPKCS1Support
        );
    }
    ...
    private endStream(datatype: P2PDataType, sendStopCommand = false): void {
        if (this.currentMessageState[datatype].p2pStreaming) {
                        ...
                        this.sendCommandWithInt(
                            {
                                ...
                                value: this.currentMessageState[datatype]
                                    .p2pStreamChannel,
                                channel:
                                    this.currentMessageState[datatype]
                                        .p2pStreamChannel,
            ...
            this.initializeMessageState(
                datatype,
                this.currentMessageState[datatype].rsaKey
            );
            this.initializeStream(datatype);
            this.closeEnergySavingDevice();
        }
        ...
        if (this.api.isConnected()) {
            try {
                ...
                if (response.status == 200) {
                    ...
                    if (result.code == 0) {
                        ...
                        dataresult.dsk_keys.forEach((key) => {
                            if (key.station_sn == this.rawStation.station_sn) {
                                ...
                                this.dskExpiration = new Date(
                                    key.expiration * 1000
                                );
                                rootP2PLogger.debug(
                                    `Get DSK keys - received key and expiration`,
                                    {
                                        stationSN: this.rawStation.station_sn,
                                        dskKey: this.dskKey,
                                        dskExpiration: this.dskExpiration,
                        ...
                        rootP2PLogger.error(
                            `Get DSK keys - Response code not ok`,
                            {
                                stationSN: this.rawStation.station_sn,
                                code: result.code,
                                msg: result.msg,
                    ...
                    rootP2PLogger.error(
                        `Get DSK keys - Status return code not 200`,
                        {
                            stationSN: this.rawStation.station_sn,
    ...
    private sendTalkbackAudioFrame(audioData: Buffer, channel: number): void {
        const messageHeader = buildCommandHeader(
            this.videoSeqNumber,
            CommandType.CMD_AUDIO_FRAME,
            P2PDataTypeHeader.VIDEO
        );
        const messageAudioHeader = buildTalkbackAudioFrameHeader(
            audioData,
            channel
        );
    ...
    private onTalkbackStreamError(err: any): void {
        const error = ensureError(err);
        rootP2PLogger.debug(`Talkback Stream Error:`, {
            error: getError(error),
            stationSN: this.rawStation.station_sn,
        });
    }
    ...
    public isTalkbackOngoing(channel: number): boolean {
        if (
            this.currentMessageState[P2PDataType.VIDEO].p2pTalkbackChannel ===
            channel
        )
