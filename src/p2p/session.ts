...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
  ...
  public async sendPing(address: Address): Promise<void> {
    if (
      (this.lastPong &&
        (new Date().getTime() - this.lastPong) / this.getHeartbeatInterval() >=
          this.MAX_RETRIES) ||
      (this.connectTime &&
        !this.lastPong &&
      ...
      if (this.connected) {
        ...
        while ((queuedMessage = this.sendQueue.shift()!) !== undefined) {
          ...
          this.messageStates.forEach((stateMessage) => {
            ...
            if (
              !stateMessage.acknowledged ||
              stateMessage.commandType === CommandType.CMD_GATEWAYINFO
            ) {
      ...
      if (!this.connected) {
        ...
        if (Device.isSmartSafe(this.rawStation.device_type)) {
          ...
          const data = Buffer.concat([
            buildCommandHeader(this.seqNumber, CommandType.CMD_GATEWAYINFO),
            payload,
          ]);
        ...
        if (message.seqNo === this.expectedSeqNo[dataType]) {
          ...
          rootP2PLogger.trace(
            `Received message - DATA ${P2PDataType[message.type]} - Received expected sequence`,
            {
              ...
              queuedDataSize:
                this.currentMessageState[dataType].queuedData.size,
            },
          );

          let queuedMessage = this.currentMessageState[dataType].queuedData.get(
            this.expectedSeqNo[dataType],
          );
          while (queuedMessage) {
            rootP2PLogger.trace(
            ...
            queuedMessage = this.currentMessageState[dataType].queuedData.get(
              this.expectedSeqNo[dataType],
            );
          }
        } else if (
          ...
          if (
            !this.currentMessageState[dataType].queuedData.get(message.seqNo)
          ) {
          ...
          if (msg_state.commandType === message.commandId) {
            ...
            if (return_code === ErrorCode.ERROR_FAILED_TO_REQUEST) {
              ...
              if (
                ...
                if (msg_state.customData) {
                  for (const index of Object.keys(this.customDataStaging)) {
                    ...
                    if (
                      new Date().getTime() -
                        this.customDataStaging[id].timestamp >
                      2 * 60 * 1000
                    ) {
              ...
              if (
                ...
                msg_state.commandType === CommandType.CMD_SDINFO_EX &&
                resultData &&
                resultData.length >= 8
              ) {
    ...
    if (!this.currentMessageState[message.dataType].invalidStream) {
      switch (message.commandId) {
        case CommandType.CMD_VIDEO_FRAME:
          ...
          const isKeyFrame =
            message.data.subarray(4, 5).readUInt8() === 1 ? true : false;
          ...
          if (videoMetaData.aesKey !== "") {
            ...
            video_data = Buffer.concat([
              decryptAESData(videoMetaData.aesKey, encrypted_data),
              unencrypted_data,
            ]);
          } else {
          ...
          if (
            ...
            if (
              ...
              this.currentMessageState[
                message.dataType
              ].p2pStreamMetadata.videoCodec = getVideoCodec(video_data);
              rootP2PLogger.trace(
              ...
              if (
                this.currentMessageState[message.dataType].p2pStreamMetadata
            ...
            if (
              ...
              this.currentMessageState[message.dataType].waitForAudioData =
                setTimeout(() => {
                  this.currentMessageState[message.dataType].waitForAudioData =
                    undefined;
                  this.currentMessageState[
          ...
          if (message.dataType === P2PDataType.VIDEO) {
            if (findStartCode(video_data)) {
              ...
              if (
                !this.currentMessageState[message.dataType].receivedFirstIFrame
              )
                this.currentMessageState[message.dataType].receivedFirstIFrame =
                  this.isIFrame(video_data, isKeyFrame);
              ...
              if (
                ...
                if (
                  this.currentMessageState[message.dataType].preFrameVideoData
                    .length > this.MAX_VIDEO_PACKET_BYTES
                )
                  this.currentMessageState[message.dataType].preFrameVideoData =
                    Buffer.from([]);
        ...
        case CommandType.CMD_AUDIO_FRAME:
          ...
          this.currentMessageState[message.dataType].audioStream?.push(
            audio_data,
          );
          break;
        default:
      ...
      switch (message.commandId) {
        ...
        case CommandType.CMD_CAMERA_INFO:
          try {
            ...
            rootP2PLogger.debug(
              ...
              { stationSN: this.rawStation.station_sn, cameraInfo: cameraData },
            );
            this.emit(
              "camera info",
              parseJSON(cameraData, rootP2PLogger) as CmdCameraInfoResponse,
            );
          } catch (err) {
            const error = ensureError(err);
            rootP2PLogger.error(
        ...
        case CommandType.CMD_DOWNLOAD_FINISH:
          rootP2PLogger.debug(
            ...
            { stationSN: this.rawStation.station_sn, channel: message.channel },
          );
          this.endStream(P2PDataType.BINARY);
          break;
        case CommandType.CMD_DOORBELL_NOTIFY_PAYLOAD:
          try {
            ...
            rootP2PLogger.debug(
              ...
              { stationSN: this.rawStation.station_sn, payload: str },
            );
          ...
          try {
            ...
            if (json !== undefined) {
              if (
                ...
                if (
                  ...
                  if (aesKey !== undefined) {
                    const decryptedPayload = decryptPayloadData(
                      ...
                      Buffer.from(
                        getLockVectorBytes(this.rawStation.station_sn),
                        "hex",
                      ),
                    ).toString();
                    rootP2PLogger.debug(
                ...
                if (
                  ...
                  if (payload.seq_num !== undefined) {
                    ...
                    rootP2PLogger.debug(
                      ...
                      {
                        stationSN: this.rawStation.station_sn,
                        lockSeqNumber: this.lockSeqNumber,
                        payload: payload,
                      },
                    );
                  }
                  ...
                  if (payload.lock_cmd > 0) {
                    if (
                      ...
                      this.emit(
                        ...
                        message.channel,
                        ESLCommand[
                          ESLBleCommand[payload.lock_cmd] as unknown as number
                ...
                json.cmd === CommandType.CMD_DOORLOCK_DATA_PASS_THROUGH
              ) {
                const payload: ESLStationP2PThroughData =
                  json.payload as ESLStationP2PThroughData;
                if (this.deviceSNs[message.channel] !== undefined) {
                  if (payload.lock_payload !== undefined) {
                    const decoded = decodeBase64(
                      decodeLockPayload(Buffer.from(payload.lock_payload)),
                    );
                    ...
                    payload.lock_payload = decryptLockAESData(
                      key,
                      iv,
                      decoded,
                    ).toString("hex");
                    ...
                    switch (payload.lock_cmd) {
                      case ESLBleCommand.NOTIFY:
                        const notifyBuffer = Buffer.from(
                      ...
                      switch (fac.getCommandCode()) {
                        ...
                        case ESLBleCommand.NOTIFY:
                          this.emit(
                            ...
                            CommandType.CMD_SMARTLOCK_QUERY_BATTERY_LEVEL,
                            parsePayload
                              .readInt8(BleParameterIndex.ONE)
                              .toString(),
                          );
                          this.emit(
                            "parameter",
                            ...
                            CommandType.CMD_SMARTLOCK_QUERY_STATUS,
                            parsePayload
                              .readInt8(BleParameterIndex.TWO)
                              .toString(),
                          );
                          break;
                        case ESLBleCommand.GET_LOCK_PARAM:
                          ...
                          this.emit(
                            ...
                            parsePayload
                              .readUint16LE(BleParameterIndex.THREE)
                              .toString(),
                          );
                          this.emit(
                            "parameter",
                            ...
                            parsePayload
                              .readInt8(BleParameterIndex.FOUR)
                              .toString(),
                          );
                          this.emit(
                            "parameter",
                          ...
                          this.emit(
                            ...
                            parsePayload
                              .readInt8(BleParameterIndex.SEVEN)
                              .toString(),
                          );
                          this.emit(
                            "parameter",
                            ...
                            parsePayload
                              .readInt8(BleParameterIndex.EIGHT)
                              .toString(),
                          );
                          this.emit(
                            "parameter",
                            ...
                            parsePayload
                              .readUint16LE(BleParameterIndex.NINE)
                              .toString(),
                          );
                          this.emit(
                            "parameter",
                            ...
                            parsePayload
                              .readInt8(BleParameterIndex.TEN)
                              .toString(),
                          );
                          this.emit(
                            "parameter",
                            ...
                            parsePayload
                              .readInt8(BleParameterIndex.ELEVEN)
                              .toString(),
                          );
                        ...
                        case ESLBleCommand.ADD_PW:
                          if (
                            ...
                            customData.customData.command.name ===
                ...
                if (payload.lock_payload !== undefined) {
                  try {
                    const fac = BleCommandFactory.parseSmartLock(
                      payload.lock_payload,
                    );
                    rootP2PLogger.debug(
                        ...
                        if (
                          ...
                          rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock  return code: ${returnCode}`,
                            {
                              ...
                              decoded: data.toString("hex"),
                              bleCommandCode:
                                functionTypeCommand[fac.getCommandCode()!],
                              returnCode: returnCode,
                            },
                          );
                          const parsePayload = new ParsePayload(
                            data.subarray(1),
                          );
                          if (
                            fac.getDataType() === SmartLockFunctionType.TYPE_2
                            ...
                            switch (fac.getCommandCode()) {
                              ...
                              case SmartLockBleCommandFunctionType2.GET_LOCK_PARAM:
                                ...
                                this.emit(
                                  ...
                                  CommandType.CMD_SMARTLOCK_AUTO_LOCK_TIMER,
                                  parsePayload
                                    .readUint16LE(BleParameterIndex.TWO)
                                    .toString(),
                                );
                                this.emit(
                                  "parameter",
                                  ...
                                  CommandType.CMD_SMARTLOCK_AUTO_LOCK_SCHEDULE,
                                  parsePayload
                                    .readInt8(BleParameterIndex.THREE)
                                    .toString(),
                                );
                                this.emit(
                                  "parameter",
                                ...
                                this.emit(
                                  ...
                                  CommandType.CMD_SMARTLOCK_ONE_TOUCH_LOCK,
                                  parsePayload
                                    .readInt8(BleParameterIndex.SIX)
                                    .toString(),
                                );
                                this.emit(
                                  "parameter",
                                ...
                                this.emit(
                                  ...
                                  CommandType.CMD_SMARTLOCK_WRONG_TRY_ATTEMPTS,
                                  parsePayload
                                    .readInt8(BleParameterIndex.NINE)
                                    .toString(),
                                );
                                this.emit(
                                  "parameter",
                                  ...
                                  CommandType.CMD_SMARTLOCK_WRONG_TRY_LOCKDOWN,
                                  parsePayload
                                    .readUint16LE(BleParameterIndex.TEN)
                                    .toString(),
                                );
                                this.emit(
                                  "parameter",
                              ...
                              case SmartLockBleCommandFunctionType2.ADD_PW:
                                if (
                                  ...
                                  this.api.updateUserPassword(
                                    customData.customData.command.value
                                      ?.deviceSN,
                                    customData.customData.command.value
                                      ?.shortUserId,
                                    parsePayload.readStringHex(
                ...
                json.cmd === CommandType.CMD_CAMERA_GARAGE_DOOR_STATUS
              ) {
        ...
        case CommandType.CMD_DATABASE_IMAGE:
          ...
          try {
            ...
            rootP2PLogger.debug(
              ...
              { stationSN: this.rawStation.station_sn, message: str },
            );
          ...
          try {
            ...
            switch (databaseResponse.cmd) {
              ...
              case CommandType.CMD_DATABASE_QUERY_LOCAL: {
                ...
                > = new SortedMap<number, Partial<DatabaseQueryLocal>>(
                  (a: number, b: number) => a - b,
                );
                for (const record of data) {
                  for (const tableRecord of record.payload) {
                    ...
                    if (record.table_name === "history_record_info") {
                      ...
                      record.table_name === "record_crop_picture_info"
                    ) {
        ...
        case CommandType.CMD_GATEWAYINFO:
          ...
          this.api
            ...
            .then((cipher) => {
              ...
              this.p2pSeqMapping.forEach(
                (value: number, key: number, map: Map<number, number>) => {
    ...
    if (this.rawStation.devices?.length > 0) {
      if (!this.energySavingDevice) {
        for (const device of this.rawStation.devices) {
          if (
            device.device_sn === this.rawStation.station_sn &&
            Device.hasBattery(device.device_type)
          ) {
