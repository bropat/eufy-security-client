...
const stringWithLength = (input: string, chunkLength = 128): Buffer => {
  ...
  const bufferSize =
    stringAsBuffer.byteLength < chunkLength
      ? chunkLength
...
export const isP2PCommandEncrypted = function (cmd: CommandType): boolean {
  return [
    1001, 1002, 1004, 1005, 1006, 1007, 1008, 1009, 1010, 1011, 1015, 1017,
    1019, 1035, 1045, 1056, 1145, 1146, 1152, 1200, 1207, 1210, 1213, 1214,
    1226, 1227, 1229, 1230, 1233, 1236, 1240, 1241, 1243, 1246, 1272, 1273,
    1275, 1400, 1401, 1402, 1403, 1408, 1409, 1410, 1412, 1413, 1506, 1507,
    1607, 1609, 1610, 1611, 1702, 1703, 1704, 1705, 1706, 1707, 1708, 1709,
    1013, 1202, 1205, 1206, 1024, 1025, 1132, 1215, 1216, 1217, 1414, 1026,
    1164, 1201, 1027, 1047, 1048, 1029, 1034, 1036, 1043, 1057, 1203, 1218,
    1219, 1220, 1221, 1222, 1223, 1224, 1232, 1234, 1235, 1237, 1238, 1248,
    1253, 1257, 1269, 1800, 1037, 1040, 1038, 1049, 1050, 1051, 1054, 1060,
    1204, 1254, 1255, 1256, 1258, 1259, 1260, 1261, 1262, 1264, 1271, 1350,
    1404, 1101, 1106, 1108, 1110, 1111, 1112, 1113, 1114, 1116, 1117, 1118,
    1119, 1121, 1103, 1129, 1211, 1228, 1231, 1242, 1249, 1250, 1251, 1252,
    1405, 1406, 1407, 1700,
  ].includes(cmd);
};

export const getP2PCommandEncryptionKey = function (
  serialNumber: string,
  p2pDid: string,
): string {
...
export const buildLookupWithKeyPayload = (
  socket: Socket,
  p2pDid: string,
  dskKey: string,
): Buffer => {
...
export const buildLookupWithKeyPayload2 = (
  p2pDid: string,
  dskKey: string,
): Buffer => {
  const p2pDidBuffer = p2pDidToBuffer(p2pDid);
...
export const buildLookupWithKeyPayload3 = (
  p2pDid: string,
  address: Address,
  data: Buffer,
): Buffer => {
...
export const buildIntCommandPayload = (
  ...
  const encrypted =
    isP2PCommandEncrypted(commandType) &&
  ...
  const strValueBuffer =
    strValue.length === 0 ? Buffer.from([]) : stringWithLength(strValue);
...
export const buildStringTypeCommandPayload = (
  encryptionType: EncryptionType,
  encryptionKey: Buffer | undefined,
  serialNumber: string,
  p2pDid: string,
...
export const buildIntStringCommandPayload = (
  ...
  const strValueBuffer =
    strValue.length === 0 ? Buffer.from([]) : stringWithLength(strValue);
  const strValueSubBuffer =
    strValueSub.length === 0 ? Buffer.from([]) : stringWithLength(strValueSub);
  const headerBuffer = Buffer.allocUnsafe(2);
  const tmpDataBuffer = Buffer.concat([
    someintBuffer,
    valueBuffer,
...
export const buildCommandHeader = (
  seqNumber: number,
  commandType: CommandType,
  p2pDataTypeHeader: Buffer | null = null,
): Buffer => {
  ...
  if (
    ...
    dataTypeBuffer = p2pDataTypeHeader as Buffer<ArrayBuffer>;
  }
...
export const buildCommandWithStringTypePayload = (
  encryptionType: EncryptionType,
  encryptionKey: Buffer | undefined,
  serialNumber: string,
  p2pDid: string,
  ...
  const encrypted =
    ...
    encryptionKey?.length === 16;
  const channelBuffer = Buffer.from([channel, encrypted ? encryptionType : 0]);
  const dataBuffer = encrypted
    ? paddingP2PData(Buffer.from(value))
    : Buffer.from(value);
  headerBuffer.writeUInt16LE(dataBuffer.length, 0);
...
export const getNewRSAPrivateKey = (
  enableEmbeddedPKCS1Support = false,
): NodeRSA => {
...
export const findStartCode = (data: Buffer): boolean => {
  if (data !== undefined && data.length > 0) {
    if (data.length >= 4) {
      ...
      if (
        (startcode[0] === 0 && startcode[1] === 0 && startcode[2] === 1) ||
        (startcode[0] === 0 &&
          startcode[1] === 0 &&
          startcode[2] === 0 &&
          startcode[3] === 1)
...
export const isIFrame = (data: Buffer): boolean => {
  ...
  if (data !== undefined && data.length > 0) {
    if (data.length >= 5) {
      ...
      if (
        validValues.includes(startcode[3]) ||
        validValues.includes(startcode[4])
      )
        return true;
    }
  }
...
export const decryptLockAESData = (
  key: string,
  iv: string,
  data: Buffer,
): Buffer => {
...
export const generateBasicLockAESKey = (
  ...
  const array: number[] = [
    104, -83, -72, 38, -107, 99, -110, 17, -95, -121, 54, 57, -46, -98, -111,
    89,
  ];
  ...
  for (let i = 0; i < 16; i++) {
    array[i] =
      array[i] +
      encStationSerial[(encStationSerial[i] * 3 + 5) % 16] +
...
export const generateLockSequence = (
  deviceType?: DeviceType,
  serialnumber?: string,
): number => {
  if (deviceType !== undefined && serialnumber !== undefined)
    if (
      Device.isLockWifi(deviceType, serialnumber) ||
      Device.isLockWifiNoFinger(deviceType)
    )
...
export const eslTimestamp = function (
  timestamp_in_sec = new Date().getTime() / 1000,
): number[] {
  const array: number[] = [];
...
export const getVideoCodec = (data: Buffer): VideoCodec => {
  if (data !== undefined && data.length > 0) {
    if (data.length >= 5) {
      ...
      const startcode = [...data.subarray(0, 5)];
      if (
        h265Values.includes(startcode[3]) ||
        h265Values.includes(startcode[4])
      ) {
...
export function isP2PQueueMessage(
  type: P2PQueueMessage | P2PMessageState,
): type is P2PQueueMessage {
  return (type as P2PQueueMessage).p2pCommand !== undefined;
}

export const encryptPayloadData = (
  data: string | Buffer,
  key: Buffer,
  iv: Buffer,
): Buffer => {
...
export const decryptPayloadData = (
  data: Buffer,
  key: Buffer,
  iv: Buffer,
): Buffer => {
...
export const buildTalkbackAudioFrameHeader = (
  ...
  const audioDataHeader = Buffer.concat([
    audioDataLength,
    unknown1,
    audioType,
    audioSeq,
...
export const decodeSmartSafeData = function (
  deviceSN: string,
  data: Buffer,
): SmartSafeNotificationResponse {
...
export const getSmartSafeP2PCommand = function (
  deviceSN: string,
  user_id: string,
  command: CommandType,
  intCommand: SmartSafeCommandCode,
  channel: number,
  sequence: number,
  data: Buffer,
): SmartSafeP2PCommandType {
  ...
  rootP2PLogger.debug(`Generate smart safe command`, {
    deviceSN: deviceSN,
    userId: user_id,
    command: command,
...
export const getLockP2PCommand = function (
  deviceSN: string,
  user_id: string,
  command: CommandType,
  channel: number,
  ...
  rootP2PLogger.debug(`Generate lock command`, {
    deviceSN: deviceSN,
    userId: user_id,
    command: command,
    channel: channel,
...
export const getLockV12P2PCommand = function (
  deviceSN: string,
  user_id: string,
  command: CommandType | ESLCommand,
  channel: number,
  lockPublicKey: string,
  sequence: number,
  data: Buffer,
  ...
  const bleCommand = new BleCommandFactory()
    ...
    .setCommandCode(
      Number.parseInt(ESLBleCommand[ESLCommand[command] as unknown as number]),
...
export const getNullTerminatedString = function (
  data: Buffer,
...
export const isCharging = function (value: number): boolean {
  return (
    isUsbCharging(value) || isSolarCharging(value) || isPlugSolarCharging(value)
  );
};

export const getSmartLockCurrentTimeInSeconds = function (): number {
  return (
    Math.trunc(new Date().getTime() / 1000) | Math.trunc(Math.random() * 100)
  );
};

export const generateSmartLockAESKey = (
  adminUserId: string,
  time: number,
): Buffer => {
...
export const getSmartLockP2PCommand = function (
  ...
  if (functionType === SmartLockFunctionType.TYPE_1) {
    commandCode = Number.parseInt(
      SmartLockBleCommandFunctionType1[
        SmartLockCommand[command] as unknown as number
      ],
    );
