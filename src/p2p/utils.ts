...
export const isP2PCommandEncrypted = function (cmd: CommandType): boolean {
    return [
        1001, 1002, 1004, 1005, 1006, 1007, 1008, 1009, 1010, 1011, 1015, 1017, 1019, 1035, 1045, 1056, 1145, 1146,
        1152, 1200, 1207, 1210, 1213, 1214, 1226, 1227, 1229, 1230, 1233, 1236, 1240, 1241, 1243, 1246, 1272, 1273,
        1275, 1400, 1401, 1402, 1403, 1408, 1409, 1410, 1412, 1413, 1506, 1507, 1607, 1609, 1610, 1611, 1702, 1703,
        1704, 1705, 1706, 1707, 1708, 1709, 1013, 1202, 1205, 1206, 1024, 1025, 1132, 1215, 1216, 1217, 1414, 1026,
        1164, 1201, 1027, 1047, 1048, 1029, 1034, 1036, 1043, 1057, 1203, 1218, 1219, 1220, 1221, 1222, 1223, 1224,
        1232, 1234, 1235, 1237, 1238, 1248, 1253, 1257, 1269, 1800, 1037, 1040, 1038, 1049, 1050, 1051, 1054, 1060,
        1204, 1254, 1255, 1256, 1258, 1259, 1260, 1261, 1262, 1264, 1271, 1350, 1404, 1101, 1106, 1108, 1110, 1111,
        1112, 1113, 1114, 1116, 1117, 1118, 1119, 1121, 1103, 1129, 1211, 1228, 1231, 1242, 1249, 1250, 1251, 1252,
        1405, 1406, 1407, 1700,
    ].includes(cmd);
};
...
export const buildIntCommandPayload = (
    ...
    const encrypted =
        isP2PCommandEncrypted(commandType) && encryptionType !== EncryptionType.NONE && encryptionKey?.length === 16;
...
export const findStartCode = (data: Buffer): boolean => {
    if (data !== undefined && data.length > 0) {
        if (data.length >= 4) {
            ...
            if (startcode[0] === 0 && startcode[1] === 0 && startcode[2] === 1) return true;
        }
    }
    return false;
};
...
export const isIFrame = (data: Buffer): boolean => {
    ...
    if (data !== undefined && data.length > 0) {
        if (data.length >= 5) {
            ...
            if (validValues.includes(startcode[3]) || validValues.includes(startcode[4])) return true;
        }
    }
    return false;
};
