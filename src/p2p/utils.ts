...
export const buildIntCommandPayload = (
    ...
    const channelBuffer = Buffer.from([
        channel,
        encrypted ? encryptionType : 0,
    ]);
...
export const buildIntStringCommandPayload = (
    ...
    const strValueSubBuffer =
        strValueSub.length === 0
            ? Buffer.from([])
            : stringWithLength(strValueSub);
    const headerBuffer = Buffer.allocUnsafe(2);
    const tmpDataBuffer = Buffer.concat([
        someintBuffer,
        valueBuffer,
...
export const buildCommandHeader = (
    ...
    if (
        ...
        (Buffer.compare(p2pDataTypeHeader, P2PDataTypeHeader.DATA) === 0 ||
            ...
            Buffer.compare(p2pDataTypeHeader, P2PDataTypeHeader.CONTROL) ===
                0 ||
...
export const generateBasicLockAESKey = (
    ...
    const array: number[] = [
        104, -83, -72, 38, -107, 99, -110, 17, -95, -121, 54, 57, -46, -98,
        -111, 89,
    ];
...
export const getLockV12P2PCommand = function (
    ...
    const bleCommand = new BleCommandFactory()
        ...
        .setCommandCode(
            Number.parseInt(
                ESLBleCommand[ESLCommand[command] as unknown as number]
            )
...
export const isCharging = function (value: number): boolean {
    return (
        isUsbCharging(value) ||
        isSolarCharging(value) ||
        isPlugSolarCharging(value)
    );
};
