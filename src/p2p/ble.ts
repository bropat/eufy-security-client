import {
    BleAdditionalDataError,
    BleCommandCodeError,
    BleDataError,
    BleDataTypeError,
    BleInvalidChecksumError,
    BleInvalidDataHeaderError,
    BleVersionCodeError,
} from './error'
...
export class BleCommandFactory {
    ...
    public static parseLockV12(data: string | Buffer): BleCommandFactory {
        ...
        fac.setResponseCode(
            fac.getPackageFlag() === -64
                ? data.readUint8(8)
                : data.readUint8(12)
        )
        fac.setData(
            data.subarray(
                fac.getPackageFlag() === -64 ? 8 : 12,
                data.length - 1
            )
        ) //TODO: Verify if position 8 is correct for data (i think it should be 9 or 13)...
        if (
            BleCommandFactory.generateHash(
                data.subarray(0, data.length - 1)
            ) !== data.readUint8(data.length - 1)
        ) {
            throw new BleInvalidChecksumError(
                'Invalid BLE data, checksum mismatch'
            )
        }
        return fac
    }
    ...
    public getLockV12Command(): Buffer {
        ...
        if (this.dataType === undefined)
            throw new BleDataTypeError(
                'BleCommandFactory data type value missing'
            )
