                ...
                case "number":
                    if (value === 0 || value === 1) {
                        ...
                        throw new InvalidPropertyValueError("Property expects a boolean value", {
                            context: { propertyName: metadata.name, propertyValue: value, metadata: metadata },
                        });
                    }
                    break;
                case "string":
...
export const validValue = function (metadata: PropertyMetadataAny, value: unknown): void {
    if (metadata.type === "number") {
        ...
        if (
            ...
            throw new InvalidPropertyValueError("Invalid value for this property according to metadata", {
                context: { propertyName: metadata.name, propertyValue: value, metadata: metadata },
            });
        }
    } else if (metadata.type === "string") {
