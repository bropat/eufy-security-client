...
export class Station extends TypedEmitter<StationEvents> {
    ...
    public setStatusLed(device: Device, value: boolean): void {
        ...
        } else if (device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8423 || device.isFloodLightT8425() || (device.getDeviceType() === DeviceType.FLOODLIGHT && !device.isFloodLightT8420X()) || device.isGarageCamera() || device.isIndoorPanAndTiltCameraS350() || (device.isSoloCameras() && this.isStationHomeBase3())) {
            this.p2pSession.sendCommandWithIntString({
                commandType: CommandType.CMD_DEV_LED_SWITCH,
                value: value === true ? 1 : 0,
                valueSub: device.getChannel(),
    ...
    public setNotificationType(device: Device, value: NotificationType | WalllightNotificationType): void {
        ...
        } else if (device.isCameraProfessional247() || device.isIndoorPanAndTiltCameraS350() || device.isFloodLightT8425() || (device.isSoloCameras() && this.isStationHomeBase3())) {
            if (!Object.values(NotificationType).includes(value as NotificationType)) {
                rootHTTPLogger.error(`The device ${device.getSerial()} accepts only this type of values:`, Object.values(NotificationType).filter((value) => typeof value === "number"));
                return;
            }
    ...
    public lockDevice(device: Device, value: boolean): void {
        ...
        } else if (device.isLockWifiT8506() || device.isLockWifiT8502() || device.isLockWifiT8510P() || device.isLockWifiT8520P() || device.isLockWifiT85L0()) {
    ...
    public startTalkback(device: Device): void {
        ...
        if ((device.isIndoorCamera() && !device.isIndoorPanAndTiltCameraS350()) || (device.isSoloCamera() && !this.isStationHomeBase3()) || (device.isFloodLight() && !device.isFloodLightT8425()) || device.isWiredDoorbell() || device.isStarlight4GLTE() || device.isWallLightCam() || device.isGarageCamera() || device.isOutdoorPanAndTiltCamera()) {
            this.p2pSession.sendCommandWithStringPayload({
                commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD,
                value: JSON.stringify({
    ...
    public setSmartLockParams(device: Device, property: PropertyName, value: PropertyValue): void {
        ...
        if (device.isLockWifiT8506() || device.isLockWifiT8502() || device.isLockWifiT8510P() || device.isLockWifiT8520P() || device.isLockWifiT85L0()) {
            let payload: Buffer;
            switch (property) {
                case PropertyName.DeviceWrongTryProtection:
    ...
    public setNotification(device: Device, value: boolean): void {
        ...
        } else if (device.isLockWifiT8506() || device.isLockWifiT8502() || device.isLockWifiT8510P() || device.isLockWifiT8520P() || device.isLockWifiT85L0() || device.isLockWifiR10() || device.isLockWifiR20()) {
            let oldvalue = 0;
    ...
    public getLockParameters(): void {
        ...
        if (Device.isLockWifiT8506(this.getDeviceType()) || Device.isLockWifiT8502(this.getDeviceType()) || Device.isLockWifiT8510P(this.getDeviceType(), this.getSerial()) || Device.isLockWifiT8520P(this.getDeviceType(), this.getSerial()) || Device.isLockWifiT85L0(this.getDeviceType())) {
