...
export class Station extends TypedEmitter<StationEvents> {
    ...
    protected constructor(
        ...
        this.p2pSession.on(
            "sequence error",
            (channel: number, command: number, sequence: number, serialnumber: string) =>
            ...
            for (const property of Object.values(metadata)) {
                if (property.key === type) {
                    try {
                        this.updateProperty(
                            property.name,
                            this.convertRawPropertyValue(property, this.rawProperties[type].value)
                        );
                    } catch (err) {
                        const error = ensureError(err);
    ...
    public setMotionDetectionTypeHB3(
        device: Device,
        type: HB3DetectionTypes | T8170DetectionTypes | SoloCameraDetectionTypes | IndoorS350DetectionTypes,
        value: boolean
    ): void {
        const propertyData: PropertyData = {
            name:
                    ...
                    value: JSON.stringify({
                        ...
                        payload: {
                            ...
                            notification_ring_onoff:
                                device.getPropertyValue(PropertyName.DeviceNotificationRing) === true ? 1 : 0,
            ...
            this.p2pSession.sendCommandWithStringPayload(
                {
                    ...
                    value: JSON.stringify({
                        ...
                        data: {
                            type:
                                value === true
                                    ? (device.getPropertyValue(PropertyName.DeviceNotificationMotion) as boolean) ===
                                      true
                                        ? 3
                                        : 1
                                    : (device.getPropertyValue(PropertyName.DeviceNotificationMotion) as boolean) ===
    ...
    public setMotionDetectionSensitivityMode(device: Device, value: number): void {
        ...
        if (
            device.getPropertyValue(PropertyName.DeviceMotionDetectionSensitivityMode) === MotionDetectionMode.STANDARD
        ) {
    ...
    public setLightSettingsManualColoredLighting(device: Device, value: RGBColor): void {
        ...
        if (
            !colors.some((color) => color.red === value.red && color.green === value.green && color.blue === value.blue)
        ) {
            throw new InvalidPropertyValueError("Invalid value for this property", {
                context: {
                    device: device.getSerial(),
    ...
    public getLockParameters(): void {
        ...
        if (
            ...
            rootHTTPLogger.debug(`Station smart lock send get lock parameters command`, {
                stationSN: this.getSerial(),
            });
            const command = getSmartLockP2PCommand(
                this.rawStation.station_sn,
