...
class Station extends tiny_typed_emitter_1.TypedEmitter {
    ...
    stopLivestream(device) {
        ...
        // For SoloCameras, E40, Floodlight 842x, Wired doorbell T8200X, WallLightCam, and GarageCamera:
        // Send CMD_DOORBELL_SET_PAYLOAD with COMMAND_END_LIVESTREAM first, then also send standard stop
        if (device.isSoloCameras() ||
            device.getDeviceType() === types_1.DeviceType.CAMERA_E40 ||
            ...
            logging_1.rootHTTPLogger.debug("Station stop livestream - sending CMD_DOORBELL_SET_PAYLOAD (END_LIVESTREAM)", {
            ...
            this.p2pSession.sendCommandWithStringPayload({
                commandType: types_2.CommandType.CMD_DOORBELL_SET_PAYLOAD,
                value: JSON.stringify({
                    commandType: types_1.ParamType.COMMAND_END_LIVESTREAM,
                    data: {}
                }),
                channel: device.getChannel()
            }, { command: commandData });
        }
        // Always send the standard stop command
